...
use App\Http\Controllers\AddScholarShip;
...
use App\Http\Controllers\Admin\StudentInfo\SmStudentAjaxController;
use App\Http\Controllers\AssignStudentScholarshipController;
use App\Http\Controllers\LeadIntegrationController;
use App\Http\Controllers\SourceController;
use App\Http\Controllers\StatusController;
use App\Http\Controllers\LeadFormController;
use App\Http\Controllers\MarkController;
use App\Http\Controllers\DatatableQueryController;
use App\Http\Controllers\MyProgramsController;
use App\Http\Controllers\MyStudentsController;
use App\Http\Controllers\SubTopicController;
use App\Http\Controllers\Programs\RoutinesController;
use App\Http\Controllers\StipendRecordsController;
use App\Http\Controllers\Student\StipendController;
use Modules\Fees\Http\Controllers\api\AjaxController;
use Modules\Lesson\Http\Controllers\SmTopicController;
...
Route::group(['middleware' => ['XSS', 'subscriptionAccessUrl']], function () {
    ...
    Route::group(['middleware' => ['CheckDashboardMiddleware']], function () {
        ...
        Route::get('class-clone/{id}', ['as' => 'class_clone', 'uses' => 'Admin\Academics\SmClassController@clone'])->middleware('userRolePermission:class');
        Route::post('class-clone', ['as' => 'class_clone_save', 'uses' => 'Admin\Academics\SmClassController@save_clone'])->middleware('userRolePermission:class');
        Route::get('class-clone/{class_id}/{section_id}/{subject_id}', ['as' => 'lesson_clone', 'uses' => 'Admin\Academics\SmClassController@lessonclone'])->middleware('userRolePermission:class');
        Route::get('topic-Clone/{id}', [SmClassController::class, 'TopicClone'])->name('TopicClone');
        Route::get('subtopic-Clone/{id}', [SmClassController::class, 'subtopics'])->name('subtopics');
        Route::get('subject-Clone/{id}', [SmClassController::class, 'subjectclone'])->name('subjectclone');

        Route::post('class-clone', ['as' => 'class_clone_save', 'uses' => 'Admin\Academics\SmClassController@save_clone'])->middleware('userRolePermission:class');
        Route::post('subject-clone', ['as' => 'subject_clone_save', 'uses' => 'Admin\Academics\SmClassController@subject_clone'])->middleware('userRolePermission:class');
        Route::post('lesson-clone', ['as' => 'lesson_clone_save', 'uses' => 'Admin\Academics\SmClassController@lesson_clone'])->middleware('userRolePermission:class');
        Route::post('topic-clone', ['as' => 'topic_clone_save', 'uses' => 'Admin\Academics\SmClassController@topic_clone'])->middleware('userRolePermission:class');
        Route::post('subtopic-clone', ['as' => 'subtopic_clone_save', 'uses' => 'Admin\Academics\SmClassController@subtopic_clone'])->middleware('userRolePermission:class');
        ...
        Route::post('subject-attendance-qrcode',  'Admin\StudentInfo\SmSubjectAttendanceController@qrcode')->name('subject-attendance-qrcode');
        ...
        Route::post('admission-queries/{id}/set-reminder', ['as' => 'admission_set_reminder', 'uses' => 'Admin\AdminSection\SmAdmissionQueryController@setReminder']);
        Route::get('/admission-queries/{id}/reminders', ['as' => 'admission_reminders', 'uses' => 'Admin\AdminSection\SmAdmissionQueryController@getReminders']);
        Route::delete('admission-queries/reminders/{id}', ['as' => 'admission_reminder_delete', 'uses' => 'Admin\AdminSection\SmAdmissionQueryController@deleteReminder']);
        Route::post('/admission-query/{id}/toggle-approval', ['as' => 'admission_query.toggle_approval', 'uses' => 'Admin\AdminSection\SmAdmissionQueryController@toggleApproval']);

        Route::post('admission-query/convert-to-student', 'Admin\AdminSection\SmAdmissionQueryController@convertToStudent')->name('admission_query.convert_to_student');

        Route::post('/admission-query-attachment', 'Admin\AdminSection\SmAdmissionQueryController@storeAttachment')->name('admission_query_attachment_store');
        Route::delete('/admission-query-attachment/{id}', 'Admin\AdminSection\SmAdmissionQueryController@deleteAttachment')->name('admission_query_attachment_delete');
        Route::get('admission-query-attachment-download/{id}', 'Admin\AdminSection\SmAdmissionQueryController@downloadAttachment')->name('admission_query_attachment_download');
        ...
        Route::post('/import-pdf', 'SmStudentAdmissionController@yimportPdf');
        Route::post('/import-csv', 'SmStudentAdmissionController@importCsv');
        ...
        Route::get('ajaxStudentPromotegroup', 'Admin\StudentInfo\SmStudentAjaxController@ajaxStudentPromotegroup');
        ...
        // Student Attendance
        Route::get('student-attendance', ['as' => 'student_attendance', 'uses' => 'Student\AttendanceController@attendancePage'])->middleware('userRolePermission:student_attendance');
        Route::get('student-attendance-request', ['as' => 'student_attendance_request', 'uses' => 'Student\AttendanceController@attendanceRequestPage']);
        ...
        Route::post('attendance-qrcode',  'Admin\StudentInfo\SmStudentAttendanceController@qrcode')->name('attendance-qrcode');
        ...
        Route::get('graduated-students', ['as' => 'graduated_students', 'uses' => 'SmStudentAdmissionController@graduatedStudent'])->middleware('userRolePermission:disabled_student');
        Route::post('graduated-students', ['as' => 'graduated_students_search', 'uses' => 'SmStudentAdmissionController@graduatedStudentSearch']);
        Route::get('graduated-student-list-datatable', 'DatatableQueryController@graduatedStudentList')->name('graduated-student-list-datatable');

        // unapproved students
        Route::get('unapproved-students', ['as' => 'unapproved_students', 'uses' => 'SmStudentAdmissionController@bladeView']);
    ...
    Route::get('badges', 'Admin\Academics\BadgeController@index')->name('badges')->middleware('userRolePermission:badges');
    Route::post('badge', 'Admin\Academics\BadgeController@store')->name('badge-store');
    Route::get('badge/{id}', 'Admin\Academics\BadgeController@edit')->name('badge-edit');
    Route::put('badge/{id}', 'Admin\Academics\BadgeController@update')->name('badge-update');
    Route::delete('badge/{id}', 'Admin\Academics\BadgeController@destroy')->name('badge-delete');
    ...
    Route::get('lead-integration', [LeadIntegrationController::class, 'index'])->name('lead_integration');
    Route::post('lead-integration', [LeadIntegrationController::class, 'store'])->name('lead_integration.store');

    Route::get('/contact', [LeadFormController::class, 'LeadToggleLightContactForm']);
    Route::get('lead-integration/lead-form', [LeadFormController::class, 'index'])->name('lead_form.index');
    Route::post('lead-integration/lead-store', [LeadFormController::class, 'store'])->name('lead_form.store');

    Route::get('lead-integration/integration-code', function () {
        return view('lead_integration_code');
    })->name('lead_integration_code-index');

    Route::get('lead-integration/source', 'App\Http\Controllers\SourceController@index')->name('source-index');
    Route::post('source', 'App\Http\Controllers\SourceController@store')->name('source-store');
    Route::get('source/{id}', 'App\Http\Controllers\SourceController@edit')->name('source-edit');
    Route::put('source/{id}', 'App\Http\Controllers\SourceController@update')->name('source-update');
    Route::delete('source/{id}', 'App\Http\Controllers\SourceController@destroy')->name('source-delete');


    Route::get('lead-integration/status', 'App\Http\Controllers\StatusController@index')->name('status-index');
    Route::post('status', 'App\Http\Controllers\StatusController@store')->name('status-store');
    Route::get('status/{id}', 'App\Http\Controllers\StatusController@edit')->name('status-edit');
    Route::put('status/{id}', 'App\Http\Controllers\StatusController@update')->name('status-update');
    Route::delete('status/{id}', 'App\Http\Controllers\StatusController@destroy')->name('status-delete');




    /**************************End Lead Integration*****************************************************************/




    // <!--------------------------------------------------------- MY STUDENTS & ROUTINES --------------------------------------------------------------------->
    ...
    Route::group(['middleware' => ['subdomain']], function () {
        Route::prefix('program')->middleware('auth')->group(function () {
            // -----------------Content Type----------------- //
            Route::get('/program-mark', [MarkController::class, "index"])->name('programs.mark');
            Route::get('/my-programs', [MyProgramsController::class, "index"])->name('programs.myprograms');
            Route::get('/my-students', [MyStudentsController::class, "classWiseSectionsListIndex"])->name('programs.my-student');
            Route::get('/routines', [RoutinesController::class, "classWiseSectionsList"])->name('programs.routines');
            Route::get('/show-my-students/{classId}/{sectionId}', [MyStudentsController::class, 'showMyStudents'])->name('show.my.students');

            Route::get('/mark-report', [MarkController::class, "markReport"])->name('mark-report');
            Route::get('/mark-report-details', [MarkController::class, "markReportDetails"])->name('mark-report-details');
        });
    });
    Route::get('/selected-class-wise-section-list', [MyStudentsController::class, "classWiseSectionsStudentsDetailsDatatable"])->name('selected_class_wise_section_list');

    Route::get('program/class-subjects/{classId}/{sectionId}', 'App\Http\Controllers\MyProgramsController@getSubject');
    Route::post('/update-topic-status', 'App\Http\Controllers\MyProgramsController@updateStatus')->name('topic.updateStatus');
    Route::post('/update-subtopic-status', 'App\Http\Controllers\MyProgramsController@updateSubtopicStatus')->name('subtopic.updateStatus');
    Route::get('/get-class-sections', 'App\Http\Controllers\MyProgramsController@getClassSections')->name('my-programs.get-class-sections');
    Route::get('/get-teacher-class-sections/{teacherId}', 'App\Http\Controllers\MyProgramsController@getTeacherClassSections');
    Route::post('/update-subject-lock', 'App\Http\Controllers\MyProgramsController@updateSubjectStatus');
    Route::post('/subtopics/update-status-bulk', 'App\Http\Controllers\MyProgramsController@updateStatusBulk')->name('subtopics.updateStatusBulk');

    // <!--------------------------------------------------------- MY STUDENTS & ROUTINES --------------------------------------------------------------------->

    Route::get('ajaxStudentClassSection', [MarkController::class, 'ajaxClassSection']);
    Route::get('ajaxClassteacherSection', [MarkController::class, 'ajaxClassteacherSection']);
    Route::get('ajaxClassteacherSubject', [MarkController::class, 'ajaxClassteacherSubject']);
    Route::get('ajaxStudentSectionSubject', [MarkController::class, 'ajaxSectionSubject']);
    Route::post('/program-mark', [MarkController::class, 'assignSubjectFind'])->name('student-search');
    Route::get('/show-my-mark/{classId}/{sectionId}/{subjectId}', [MarkController::class, 'showMyMarks']);
    Route::post('/teacher-mark', [MarkController::class, 'assignSubjectTeacherFind'])->name('teacher-search');
    Route::post('/lock-request/{id}/{action}', [MarkController::class, 'lockRequest'])->name('lock-request');
    Route::get('/get-subjects', [MarkController::class, 'getSubjects']);
    Route::post('/save-marks', [MarkController::class, 'saveMarks'])->name('save.marks');
    Route::patch('/assign-subject/{id}/{action}', [MarkController::class, 'updateStatus']);
    Route::post('update.mark.lock', [MarkController::class, 'updateMarkLock'])->name('update.mark.lock');
    Route::get('/download-pdf', [MarkController::class, 'downloadPdf'])->name('download.pdf');
    Route::get('/subject/download-pdfs/{id}', [MarkController::class, 'downloadPDFs']);
    Route::get('ajaxStudentSectionStudent', [MarkController::class, 'ajaxSectionStudent']);
    Route::post('/export-students', [MarkController::class, 'exportStudents'])->name('export-students');
    Route::post('/generate-pdf', 'MarkController@marktablePdf')->name('marktable.pdf');
    Route::post('/marktable/csv', [MarkController::class, 'marktableCsv'])->name('marktable.csv');
    Route::post('/marktable/text', [MarkController::class, 'downloadTableAsText'])->name('marktable.text');


    Route::get('ajaxStudentClassmultipleSection', [MarkController::class, 'ajaxClassMultipleSection']);
    ...
    Route::post('/enable-sub-topic', [SmTopicController::class, 'EnableSubTopic']);

    // <!--------------------------------------------------- SUB TOPIC ------------------------------->

    Route::get('sub-topic', [SubTopicController::class, 'index'])->name('sub-topic');
    Route::post('sub-topic', [SubTopicController::class, 'store'])->name('sub-topic-store');
    Route::get('get-all-subtopics-ajax', [SubTopicController::class, 'getAllSubTopicsAjax'])->name('get-all-subtopics-ajax');
    Route::get('subtopic-edit/{id}', [SubTopicController::class, 'edit'])->name('sub-topic-edit');
    Route::post('subtopic-update', [SubTopicController::class, 'updateSubTopic'])->name('sub-topic.update');
    Route::post('subtopic-delete', [SubTopicController::class, 'subtopicdelete'])->name('subtopic-delete');
    Route::get('subtopicTitle-delete/{id}', [SubTopicController::class, 'deletesubTopic'])->name('subtopicTitle-delete');
    Route::post('sub-topic-edit-image-delete', [SubTopicController::class, 'SubTopicEditImageDelete'])->name('sub-topic-edit-image-delete');
    // <!------------------------------------------------------------- LESSON BASED TOPICS ------------------------------------------------------>

    Route::get('/lesson/adminajaxSelectTopic', [SubTopicController::class, 'ajaxSelectTopic']);

    // <!------------------------------------------------------------- LESSON BASED TOPICS ------------------------------------------------------>

    // <!--------------------------------------------------- SUB TOPIC ------------------------------->
    Route::get('marksheet-details', function () {
        return view('programs.marksheet');
    });

    Route::group(['middleware' => ['subdomain']], function () {
        Route::prefix('scholarships')->middleware('auth')->group(function () {
            // -----------------Content Type----------------- //
            Route::get('/add-scholarship', [AddScholarShipController::class, "index"])->name('add-scholarship');
            Route::get('/assign-student', [AssignStudentScholarshipController::class, "index"])->name('assign-student');

            Route::get('/add-stipend', [StipendController::class, "stipendPage"])->name('add-stipend');
            Route::get('/stipend-records', [StipendRecordsController::class, 'index'])->name('stipend-records');
            Route::get('/stipend-records-datatable', [StipendRecordsController::class, 'stipendrecordsdatatable'])->name('stipend-records-datatable');
            Route::get('stipend-records-payment/{id}', [StipendRecordsController::class, 'addstipendrecordsPayment'])->name('stipend-records-payment')->middleware('userRolePermission:fees.add-fees-payment');
            Route::get('stipend-records-view/{id}/{state}', [StipendRecordsController::class, 'stipendRecordsView'])->name('stipend-records-view');
        });
    });
    Route::post('/adding-scholarships', [AddScholarShipController::class, 'store'])->name('adding-scholarships');
    Route::get('/edit-scholarships/{id}', [AddScholarShipController::class, 'edit'])->name('edit-scholarships');
    Route::post('/update-scholarships/{id}', [AddScholarShipController::class, 'update'])->name('update-scholarships');
    Route::get('/delete-scholarships/{id}', [AddScholarShipController::class, 'destroy'])->name('delete-scholarships');
    Route::post('/stipend-records-payment-post/{id}', [StipendRecordsController::class, 'stipendrecordspayment'])->name('stipend-records-payment-post');
    Route::post('stipend-records-delete', [StipendRecordsController::class, 'admissionfeesInvoiceDelete'])->name('stipend-records-delete');


    // <-------------   Assign Student Scholarship  ---------> //
    Route::get('/get-group-students-list', [AssignStudentScholarshipController::class, 'getStudentsList'])->name('group-students-list');
    Route::get('/get-groups-list', [AssignStudentScholarshipController::class, 'getGroupsList']);
    Route::get('/get-sections-list', [AssignStudentScholarshipController::class, 'getSectionsList']);
    Route::post('/assign-students-scholarship', [AssignStudentScholarshipController::class, 'store'])->name('assign-scholarship');
    Route::get('assign-student-scholarship', 'App\Http\Controllers\AssignStudentScholarshipController@AssignStudentScholarshipDatatable')->name('assign-student-scholarship-datatable');
    Route::get('/edit-student-scholarship/{id}', [AssignStudentScholarshipController::class, 'edit'])->name('edit-student-scholarships');
    Route::post('/update-student-scholarship', [AssignStudentScholarshipController::class, 'update'])->name('update-student-scholarship');
    Route::get('delete-student-scholarship', [AssignStudentScholarshipController::class, 'studentScholarshipDelete'])->name('delete-student-scholarship');
    Route::post('scholarship-student-search', 'App\Http\Controllers\AssignStudentScholarshipController@ScholarshipStudentSearch')->name('scholarship-student-search');
});
