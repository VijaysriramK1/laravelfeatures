...
<section class="sms-breadcrumb mb-20">
    <div class="container-fluid">
        <div class="row justify-content-between">
            <h1>Stipend Amount Payment</h1>
            <div class="bc-pages">
                <a href="{{route('dashboard')}}">@lang('common.dashboard')</a>
                <a href="#">Stipend Record </a>
                @if (isset($role) && $role =='admin')
                <a href="{{route('fees.fees-invoice-list')}}">Stipend Amount Payment</a>
                @elseif(isset($role) && $role =='student')
                <a href="#">Stipend Record</a>
                @endif
                <a href="#">Stipend Amount Payment</a>
            </div>
        </div>
    </div>
</section>
<section class="admin-visitor-area up_st_admin_visitor student-details">
    <div class="container-fluid p-0">
        @if (isset($role) && $role =='admin')
        {{ Form::open(['class' => 'form-horizontal', 'method' => 'POST', 'route' => ['stipend-records-payment-post', $invoiceInfo->id], 'enctype' => 'multipart/form-data']) }}
            ...
            <div class="col-lg-3">
                ...
                <div class="student-meta-box">
                    ...
                    <img class="student-meta-img img-100"
                        src="{{ isset($invoiceInfo->studentInfo) && $invoiceInfo->studentInfo->student_photo ? $invoiceInfo->studentInfo->student_photo : asset('public/uploads/staff/demo/staff.jpg') }}"
                        alt="">
                    ...
                    <div class="white-box radius-t-y-0">
                        <div class="single-meta mt-50">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="value">{{$invoiceInfo->student->full_name}}</div>
                            </div>
                        </div>
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                <div class="name">@lang('student.admission_number')</div>
                                <div class="value">{{$invoiceInfo->student->admission_no}}</div>
                            </div>
                        </div>
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                <div class="name">@lang('student.roll_number')</div>
                                <div class="value">{{$invoiceInfo->student->roll_no}}</div>
                            </div>
                        </div>
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                <div class="name">@lang('common.class')</div>
                                <div class="value">{{$invoiceInfo->record->class->class_name}}</div>
                            </div>
                        </div>
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                <div class="name"> @lang('common.section')</div>
                                <div class="value">{{$invoiceInfo->record->section->section_name}}</div>
                            </div>
                        </div>
            ...
            <div class="col-lg-9">
                <div class="row">
                    <div class="col-lg-4 no-gutters">
                        <div class="main-title">
                            <h3 class="mb-0">Stipend Amount  </h3>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="big-table">
                                ...
                                <table class="table school-table-style p-0" cellspacing="0" width="100%">
                                    ...
                                    <tbody>
                                        ...
                                        <input type="hidden" class="weaverType" value="amount">
                                        <input type="hidden" name="student_id" value="{{$invoiceInfo->student->id}}">
                                        @foreach ($invoiceDetails as $key=>$invoiceDetail)
                                        <tr>
                                            <td>{{$key + 1}}</td>
                                            <input type="hidden" name="fees_type[]"
                                                value="{{$invoiceDetail->fees_type}}">
                                            <td> {{ $invoiceInfo->stipends->scholarship->name ?? null}}</td>
                                            <td>{{ $invoiceInfo->end_date ? \Carbon\Carbon::parse($invoiceInfo->end_date)->format('Y-m-d') : 'N/A' }}</td>
                                            ...
                                            <td>
                                                <div class="primary_input">
                                                    <input class="primary_input_field border-0 form-control addFeesAmount{{ $errors->has('amount') ? ' is-invalid' : '' }}"
                                                        ...
                                                        value="{{ $invoiceInfo->stipends->scholarship->coverage_amount ?? 'N/A' }}"
                                            ...
                                            <td>
                                                <div class="primary_input">
                                                    <input class="primary_input_field border-0 form-control addFeesAmount{{ $errors->has('amount') ? ' is-invalid' : '' }}"
                                                        type="text" name="amount[]" autocomplete="off"
                                                        value="{{isset($invoiceDetail)? $invoiceDetail->stipends_amount: old('stipends_amount')}}"
                                                        readonly>
                                            ...
                                            <td>
                                                <div class="primary_input">
                                                    <input class="primary_input_field border-0 form-control showTotalValue"
                                                        type="text" name="due[]"
                                                        value="{{ isset($dueamount) ? $dueamount : '' }}"
                                                        autocomplete="off" readonly>
                                                    <input class="dueAmount" type="hidden"
                                                        value="{{ isset($dueamount) ? $dueamount : 0 }}">
