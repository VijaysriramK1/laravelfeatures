                                        ...
                                        <table>
                                            <thead class="table-bordered">
                                                ...
                                                <tr style="background-color:#F6F8FA;">
                                                    ...
                                                    @if($topic->subtopics->count() > 0)
                                                    <th class="text-center" colspan="{{ $topicColspan }}" id="header_{{ $topic->id }}" data-cgpa="{{ $topic->cgpa ?? 1 }}" data-max_marks="{{ $topic->max_marks }}" data-topic-id="{{ $topic->id }}">
                                                        {{ $topic->topic_title }} - {{ $topic->cgpa ?? '' }} {{ $topic->unit ?? '' }}<br style="text-align: center;">{{ $topic->max_marks ?? '' }}
                                                    </th>
                                                    @else
                                                    <th class="text-center" rowspan="2" id="header_{{ $topic->id }}" data-cgpa="{{ $topic->cgpa ?? 1 }}" data-max_marks="{{ $topic->max_marks }}" data-topic-id="{{ $topic->id }}">
                                                        {{ $topic->topic_title }} - {{ $topic->cgpa ?? '' }} {{ $topic->unit ?? '' }}<br style="text-align: center;">{{ $topic->max_marks ?? '' }}
                                                    </th>
                                                    @endif
                                                    @endforeach
                                                ...
                                                <tr>
                                                    ...
                                                    @php $totalMarks += $subtopic->max_marks; @endphp
                                                    @if($lesson->topics->count() > 0)
                                                    <th class="text-center" data-max_marks="{{ $subtopic->max_marks }}" data-subtopic-id="{{ $subtopic->id }}"
                                                        id="subtopic_{{ $subtopic->id }}">
                                                        {{ $subtopic->sub_topic_title }}<br>{{ $subtopic->max_marks }}
                                                    </th>
                                                    @endif
                                            ...
                                            <tbody style="border-right: none;">
                                                ...
                                                <tr>
                                                    ...
                                                    <td id="overall_{{ $student->id }}" style="text-align: center;">
                                                        @if($studentmarkdata->isEmpty())
                                                        <!-- If there are no records, display 0 -->
                                                        0
                                                        @else
                                                        @php
                                                        $studentmark = $studentmarkdata->first(function ($mark) use ($student) {
                                                        return $mark->student_id == $student->id;
                                                        });
                                                        @endphp
