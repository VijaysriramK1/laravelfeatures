            ...
            @foreach ($get_classwise_sections as $get_values)
                <div class="tab-item tabId_{{ $get_values->class->id }}_{{ $get_values->section->id }}">
                    <a class="text-decoration-none classwisetable active-tab-hover" href="javascript:void(0);"
                        ...
                        <b><span>{{ strtoupper($get_values->class->class_name) ?? '' }}<span>
                                    <span>(</span>{{ strtoupper($get_values->section->section_name) ?? '' }}<span>)</span>
                        </b></a>
                </div>
            @endforeach
        @else
                    ...
                    fetch('/calendar-view-data')
                        ...
                        .then(data => {
                            var events = data.map(function(event) {
                                return {
                                    title: 'Class & Section: ' + event.class_section + '<br>' + 'Subject Name: ' + event.title + '<br>' + 'Start Time: ' + event.start_time + '<br>' + 'End Time: ' + event.end_time + '<br>' + 'Schedule Day: ' + event.day + '<br>' + 'Completed Date: ' + event.completed_date,
                                    start: event.start,
                                    end: event.end
                                };
                            });
