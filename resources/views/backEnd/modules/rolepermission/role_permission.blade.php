...
@section('mainContent')
    ...
    <div class="erp_role_permission_area ">
        ...
        {{ Form::open([
            ...
            'files' => true,
            'url' => 'rolepermission/role-permission-assign',
            'method' => 'POST',
        ]) }}
            ...
            @foreach ($all_modules as $key => $row)
                @php
                    if (moduleStatusCheck('SaasRolePermission') == true) {
                        $module_info = Modules\RolePermission\Entities\SmaModuleInfo::where('module_id', $key)->where('active_status', 1)->first();
                        ...
                        $module_info = Modules\RolePermission\Entities\SmaModuleInfo::where('parent_id', 0)->where('module_id', $key)->where('is_saas', 0)->where('active_status', 1)->first();
                        $all_group_modules = Modules\RolePermission\Entities\SmaModuleInfo::where('module_id', $key)->where('id', '!=', $key)->where('is_saas', 0)->where('active_status', 1)->get();
                    }
                    $check_all = 1;
                        ...
                        <div class="permission_header d-flex align-items-center justify-content-between">
                            <div>
                                <input type="checkbox" name="module_id[]" value="{{ $module_info->id }}"
                                    ...
                                    {{ in_array($module_info->id, $already_assigned) ? 'checked' : '' }}>
        ...
        <div class="row mt-40">
            <div class="col-lg-12 text-center">
                ...
                @if (Illuminate\Support\Facades\Config::get('app.app_sync') && $role->id < 10)
                    <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Disabled For Demo "> <button
                            class="primary-btn fix-gr-bg  demo_view" style="pointer-events: none;"
                            type="button">@lang('common.submit') </button></span>
                @else
