...
@section('mainContent')
                            ...
                            <ul class="nav nav-tabs tabs_scroll_nav mb-4 mt-2" role="tablist" id="tabs_ul">
                                ...
                                <li class="nav-item edit-button">
                                    @if (userPermission('add-new-class-routine-store'))
                                        <button class="primary-btn small fix-gr-bg" onclick="addRowInRoutine();"
                                            id="addRowBtn">
        ...
        <tr id="row_${row_count}" class="0">
            <td class="border-top-0"> 
                <div class="primary_input">
                    <select class="form-control selectSubject" name="routine[${row_count}][subject]" data-subject_row_id="${row_count}" id="subject_${row_count}" required>
                         <option value="" hidden>@lang('academics.subject')</option>
            ...
            <td class="border-top-0"> 
                <div class="row " id="teacher-div">
                    <div class="col-lg-12">
                        <select class="form-control selectTeacher teacher_select_cls_${row_count}" data-teacher_row_id="${row_count}" name="routine[${row_count}][teacher_id]"  id="teacher_${row_count}" required>
    ...
    <td class="border-top-0">
        <div class="primary_input">
        <input type="date" class="form-control primary_input_field {{ @$errors->has('start_date') ? ' is-invalid' : '' }}" name="routine[${row_count}][start_date]"
            id="start_date_${row_count}" onchange="callDateMethod(${row_count})" required>
    ...
    <td class="border-top-0">
        <div class="primary_input">
        <input type="date" class="form-control primary_input_field {{ @$errors->has('end_date') ? ' is-invalid' : '' }}" name="routine[${row_count}][end_date]"
            id="end_date_${row_count}" required  min="">
    ...
    <td class="border-top-0">  
            ...
            <td class="border-top-0">
                ...
                function isBusy(id, type) {
                    ...
                    let start_date = $('#start_date_' + id).val();
                    let end_date = $('#end_date_' + id).val();
                ...
                $(document).on('change', '.selectSubject', function(e) {
                    var getId = $(this).data('subject_row_id');
                    $('.teacher_select_cls_' + getId).find('option').not(':first').remove();
                    var classId = $('#select_class').val();
                    var sectionId = $('#select_section').val();
                    var subjectId = $('#subject_' + getId).val();
                    $.ajax({
                        url: '/selected-subject-teacher-list?class_id=' + classId + '&section_id=' + sectionId +
                            '&subject_id=' + subjectId,
                        method: 'GET',
                        success: function(response) {
                            response.forEach(function(item) {
                                $('.teacher_select_cls_' + getId).append("<option value='" + item
                                    .teacher.id + "'>" + (item.teacher.first_name ? item.teacher
                                        .first_name : '') + ' ' + (item.teacher.last_name ? item
