...
@section('mainContent')
                            ...
                            <select class="primary_select form-control{{ $errors->has('exam') ? ' is-invalid' : '' }}"
                                ...
                                @foreach ($exams as $exam)
                                    <option value="{{ $exam->id }}"
                                        {{ isset($exam_id) ? ($exam_id == $exam->id ? 'selected' : '') : '' }}>{{ $exam->title }}
                                    </option>
                                @endforeach
                            </select>
                            @if ($errors->has('exam'))
                                            ...
                                            <div class="card-header">
                                                <div class="d-flex">
                                                    ...
                                                    <div class="col-lg-6 ml-30">
                                                        ...
                                                        <p class="text-white mb-0">Email:
                                                            {{ isset($email) ? $email : 'admin@demo.com' }} , Phone:
                                            ...
                                            <div class="card-body">
                                                <div class="col-md-12">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-lg-4 text-black">
                                                            ...
                                                            @if (@$marks_grade)
                                                                <table
                                                                    class="table  table-bordered table-striped text-black"
                                                                    id="grade_table">
                                                                    <thead>
                                                                        <tr>
                                               ...
                                               @endphp --}}
                                                                <tr>
                                                                    ...
                                                                    <td width='15%' class="subject-name">
                                                                        {{ $data->subject->subject_name }} </td>
                                                                    ...
                                                                    <td width='10%'>
                                                                        ...
                                                                        @endphp
                                                                        @if (@$optional_subject == $data->subject_id) (GPA above {{ @$optional_subject_setup->gpa_above }})
                                                        <hr>
                                                        @if ($optional_subject_setup != '')
    ...
    @endif
    @if (@$optional_subject == $data->subject_id) {{ @$mark_grade->gpa - $optional_subject_setup->gpa_above }}
                                                        @else
                                                            {{ @$mark_grade->gpa }} @endif
    ...
    @if ($optional_subject_setup != '')
        @if ($subject_count == 2)
            <td rowspan="{{ count($subjects) }}" style="vertical-align: middle">
                {{ App\SmAssignSubject::get_student_result($student_detail->id, $data->subject_id, $class_id, $section_id, $exam_type_id, $optional_subject_id, $optional_subject_setup) }}
            </td>
    ...
    @if ($subject_count == 2)
        <td rowspan="{{ count($subjects) }}" style="vertical-align: middle">
            {{ App\SmAssignSubject::get_student_result_without_optional($student_detail->id, $data->subject_id, $class_id, $section_id, $exam_type_id, $optional_subject_id, $optional_subject_setup) }}
        </td>
    ...
    <div class="row">
        <div class="col-lg-12">
            <p class="result-date">
                @php
                    $data = App\SmMarkStore::select('created_at')
                        ->where([['student_id', $student_detail->id], ['class_id', $class_id], ['section_id', $section_id], ['exam_term_id', $exam_type_id]])
                        ->first();
