                    ...
                    <div class="white-box">
                        ...
                        <div class="row">
                            <div class="col-lg-3">
                                ...
                                <select
                                    class="primary_select form-control{{ $errors->has('current_session') ? ' is-invalid' : '' }}"
                                    name="current_session" id="academic_year">
                                    <option data-display="@lang('student.select_academic_year') *" value="">@lang('student.select_academic_year') *</option>
                                    @foreach ($sessions as $session)
                                        <option value="{{ $session->id }}"
                                            {{ isset($current_session) ? ($current_session == $session->id ? 'selected' : '') : '' }}>
                            ...
                            <div class="col-lg-3">
                                ...
                                <select
                                    class="primary_select form-control{{ $errors->has('promote_session') ? ' is-invalid' : '' }}"
                                    ...
                                    @foreach ($sessions as $session)
                                        <option value="{{ $session->id }}"
                                            {{ isset($promote_session) ? ($promote_session == $session->id ? 'selected' : '') : '' }}>
                                            {{ $session->year }} [{{ $session->title }}]</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-lg-3 mt-30-md">
                                ...
                                <select
                                    class="primary_select form-control{{ $errors->has('current_class') ? ' is-invalid' : '' }}"
                                    id="classSelectStudent" name="current_class">
                                    ...
                                    @if (isset($currrent_academic_class))
                                        @foreach ($currrent_academic_class as $class)
                                            <option value="{{ $class->id }}"
                                                {{ isset($current_class) ? ($current_class == $class->id ? 'selected' : '') : '' }}>
                            ...
                            <div class="col-lg-3 mt-30-md" id="sectionStudentDiv">
                                ...
                                <select
                                    class="primary_select  form-control{{ $errors->has('current_section') ? ' is-invalid' : '' }}"
                                    id="sectionSelectStudent" name="current_section">
                                    <option data-display="@lang('student.select_section')*" value="">@lang('student.select_section')</option>
                                    @isset($sections)
                                        @foreach ($sections as $section)
                                            <option value="{{ $section->sectionName != '' ? $section->sectionName->id : '' }}"
                                                {{ isset($current_section) ? ($current_section == ($section->sectionName != '' ? $section->sectionName->id : '') ? 'selected' : '') : '' }}>
            ...
            <div class="container-fluid p-0">
                <div class="row mt-40 white-box">
                    <div class="col-lg-12">
                            ...
                            <div class="col-lg-12 no-gutters">
                                <div class="main-title">
                                    <h3 class="mb-30">@lang('student.promote') |
                                        <small>
                                            @lang('student.academic_year') :
                                            {{ $search_current_academic_year != '' ? $search_current_academic_year->year . '[' . $search_current_academic_year->title . ']' : '' }},
                                            @lang('common.class'):
                                            {{ $search_current_class != '' ? $search_current_class->class_name : ' ' }},
                                            @lang('common.section'):
                                            {{ $search_current_section != '' ? $search_current_section->section_name : ' ' }},
                                            <strong> @lang('student.promote_academic_year') </strong>:
                                            {{ $search_promote_academic_year != '' ? $search_promote_academic_year->year . '[' . $search_promote_academic_year->title . ']' : '' }}
                                        </small>
                                    </h3>
                                </div>
                                @if ($errors->any())
                        ...
                        <input type="hidden" name="pre_section"
                            value="{{ isset($current_section) ? $current_section : '' }}">
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                @if (generalSetting()->badge_system)
                                                    <th class="class_datas">@lang('admin.badge')</th>
                                                @endif
                                                ...
                                                <tr>
                                                    <td>
                                                        <input type="checkbox" id="student_{{ $student->id }}"
                                                            class="common-checkbox promote_check"
                                                            name="promote[{{ $student->id }}][student]"
                                                            value="{{ $student->id }}">
                                                        <label for="student_{{ $student->id }}"></label>
                                                    </td>

                                                    <td> <a href="{{ route('student_view', [$student->id]) }}"
                                                            target="_blank" rel="noopener noreferrer">
                                                            <h5 style="color:#A235EC">
                                                                {{ $student->studentRecord->getRawOriginal('roll_no') }}
                                                            </h5>
                                                        </a> </td>
                                                    <td>{{ $student->first_name . ' ' . $student->last_name }}</td>
                                                    <td>
                                                        <select class="primary_select form-control promote_type"
                                                            data-student-id="{{ $student->id }}">
                                                            <option data-display="@lang('student.select_promotion_type') *" value="">
                                                                @lang('student.select_promotion_type') *</option>
                                                            <option value="next_class"
                                                                {{ $student->studentRecords->isNotEmpty() || ($student->studentRecords->first() && $student->studentRecords->first()->is_graduate == 0) ? 'selected' : '' }}>
                                                                {{ __('student.next_class') }} </option>
                                                            <option value="graduate"
                                                                {{ $student->studentRecords->isNotEmpty() && $student->studentRecords->first()->is_graduate == 1 ? 'selected' : '' }}>
                                                                {{ __('student.graduate') }}</option>
                                                        </select>
                                                    </td>

                                                    <td class="graduate_datas d-none text-center"
                                                        data-student-id="{{ $student->id }}">
                                                        <input type="checkbox" name="is_graduate" value="1"
                                                            class="is_graduate_checkbox"
                                                    ...
                                                    <td class="class_datas">
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                ...
                                                                <select
                                                                    class="primary_select form-control {{ $errors->has('class') ? ' is-invalid' : '' }} promote_class"
                                                                    id="promote_class" data-key="{{ $key }}"
                                                                    name="promote[{{ $student->id }}][class]">
                                                                    <option data-display="@lang('student.select_class') *"
                                                                        value="">@lang('student.select_class') *</option>
                                                                    @foreach ($classes as $class)
                                                                        <option value="{{ @$class->id }}"
                                                                            {{ $next_class && $next_class->id == $class->id ? 'selected' : '' }}>
                                                                            {{ $class->class_name }}</option>
                                                                    @endforeach
                                                                </select>
                                                                @if ($errors->has('class'))
                                                    ...
                                                    <td class="class_datas">
                                                        <div class="row">
                                                            <div class="col-lg-12"
                                                                id="promote_section_div{{ $key }}">
                                                                <select
                                                                    class="primary_select form-control{{ $errors->has('section') ? ' is-invalid' : '' }} promote_section"
                                                                    id="promote_section{{ $key }}"
                                                                    name="promote[{{ $student->id }}][section]">
                                                                    ...
                                                                    @if ($next_sections)
                                                                        @foreach ($next_sections as $section)
                                                                            <option
                                                                                value="{{ $section->sectionWithoutGlobal->id }}">
                                                                                {{ $section->sectionWithoutGlobal->section_name }}
                                                                            </option>
                                                                        @endforeach
                                                                    @endif
                                                                </select>
                                                                <div class="pull-right loader loader_style select_section_promote"
                                                                    id="select_section_promote{{ $key }}">
                                                    ...
                                                    @if (generalSetting()->badge_system)
                                                        <td>
                                                            <div class="row">
                                                                <div class="col-lg-12"
                                                                    id="promote_badge_div{{ $key }}">
                                                                    <select
                                                                        class="primary_select form-control{{ $errors->has('badge') ? ' is-invalid' : '' }} promote_badge"
                                                                        id="promote_badge{{ $key }}"
                                                                        name="promote[{{ $student->id }}][badge]">
                                                                        <option data-display="@lang('admin.badge') *"
                                                                            value="">@lang('admin.badge') *
                                                                        </option>
                                                                        @if ($badges)
                                                                    ...
                                                                    <div class="pull-right loader loader_style select_badge_promote"
                                                                        id="select_badge_promote{{ $key }}">
                                                                        <img class="loader_img_style"
                                                                            src="{{ asset('public/backEnd/img/demo_wait.gif') }}"
                                                                            alt="loader">
                                                                    </div>
                                                        ...
                                                        <div class="row">
                                                            <div class="col-lg-12">
                                                                <div class="primary_input">
                                                                    <input
                                                                        class="primary_input_field form-control{{ @$errors->has('name') ? ' is-invalid' : '' }} promote_roll_number"
                                                                        type="number"
                                                                        name="promote[{{ $student->id }}][roll_number]"
                                                                        autocomplete="off">
        ...
        $(document).ready(function() {
            ...
            $(document).on('keyup', '.promote_roll_number', function() {
                ...
                if (class_id == '') {
                    ...
                    $(this).closest('tr').find('.errorExitRoll').delay(3000).fadeOut('slow').html(
                        class_error_msg);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data > 0) {
                            ...
                            $this.closest('tr').find('.errorExitRoll').delay(5000).fadeOut(
                                'slow').html(error_msg);
                        }
                    },
                    error: function(data) {
