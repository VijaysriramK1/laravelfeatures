...
@section('mainContent')
                                    ...
                                    <div class="row no-gutters input-right-icon">
                                        <div class="col">
                                            <div class="primary_input">
                                                <input
                                                    class="primary_input_field  primary_input_field date form-control form-control{{ $errors->has('attendance_date') ? ' is-invalid' : '' }} {{ isset($date) ? 'read-only-input' : '' }}"
                            ...
                            @else
                                    ...
                                    <select
                                        ...
                                        @foreach ($classes as $class)
                                            <option value="{{ $class->id }}"
                                                {{ isset($input['class']) ? ($input['class'] == $class->id ? 'selected' : '') : '' }}>
                                        ...
                                        @isset($sections)
                                            @foreach ($sections as $section)
                                                <option value="{{ $section->section_id }}"
                                                    {{ isset($input['section']) ? ($input['section'] == $section->section_id ? 'selected' : '') : '' }}>
                                ...
                                <div class="col-lg-3 mt-30-md" id="select_subject_div">
                                    <select
                                        ...
                                        <option data-display="{{ __('student.select_subject') }} *" value="">
                                            {{ __('student.select_subject') }} *</option>
                                        @isset($subjects)
                                            @foreach ($subjects as $subject)
                                                @php
                                                    $type =
                                                        $subject->subject->subject_type == 'T' ? 'Theory' : 'Practical';
                                                @endphp
                                                <option value="{{ $subject->subject_id }}"
                                                    {{ isset($input['subject']) ? ($input['subject'] == $subject->subject_id ? 'selected' : '') : '' }}>
                                                    {{ $subject->subject->subject_name }} ({{ $type }})</option>
                                            @endforeach
                                        @endisset
                                    </select>
                                            ...
                                            <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>{{ $student->studentDetail->admission_no }}
                                                            <input type="hidden" name="attendance[{{ $student->id }}]"
                                                                value="{{ $student->id }}">
                                                            <input type="hidden"
                                                                name="attendance[{{ $student->id }}][student]"
                                                                value="{{ $student->student_id }}">
                                                            <input type="hidden"
                                                                name="attendance[{{ $student->id }}][class]"
                                                                value="{{ $student->class_id }}">
                                                            <input type="hidden"
                                                                name="attendance[{{ $student->id }}][section]"
                                                            ...
                                                            <div class="d-flex radio-btn-flex">
                                                                <div class="mr-20">
                                                                    <input type="radio"
                                                                        ...
                                                                        value="P"
                                                                        class="common-radio attendanceP subject_attendance_type"
                                                                        {{ $student->studentDetail->DateSubjectWiseAttendances != null ? ($student->studentDetail->DateSubjectWiseAttendances->attendance_type == 'P' ? 'checked' : '') : 'checked' }}>
                                                                    <label
                                                                ...
                                                                <div class="mr-20">
                                                                    <input type="radio"
                                                                        name="attendance[{{ $student->id }}][attendance_type]"
                                                                        id="attendanceL{{ $student->id }}"
                                                                        value="L"
                                                                        class="common-radio subject_attendance_type"
                                                                        {{ $student->studentDetail->DateSubjectWiseAttendances != null ? ($student->studentDetail->DateSubjectWiseAttendances->attendance_type == 'L' ? 'checked' : '') : '' }}>
                                                                    <label
                                                                ...
                                                                <div class="mr-20">
                                                                    <input type="radio"
                                                                        ...
                                                                        value="A"
                                                                        class="common-radio subject_attendance_type"
                                                                        {{ $student->studentDetail->DateSubjectWiseAttendances != null ? ($student->studentDetail->DateSubjectWiseAttendances->attendance_type == 'A' ? 'checked' : '') : '' }}>
                                                                    <label
                                                                ...
                                                                <div>
                                                                    <input type="radio"
                                                                        name="attendance[{{ $student->id }}][attendance_type]"
                                                                        id="attendanceH{{ $student->id }}"
                                                                        value="F"
                                                                        class="common-radio subject_attendance_type"
                                                                        {{ $student->studentDetail->DateSubjectWiseAttendances != null ? ($student->studentDetail->DateSubjectWiseAttendances->attendance_type == 'F' ? 'checked' : '') : '' }}>
                                                                    <label
                                                        ...
                                                        <td>
                                                            <div class="primary_input">
                                                                <textarea class="primary_input_field form-control note_{{ $student->id }}" cols="0" rows="2"
                                                                    name="attendance[{{ $student->id }}][note]">{{ $student->studentDetail->DateSubjectWiseAttendances != null ? $student->studentDetail->DateSubjectWiseAttendances->notes : '' }}</textarea>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                @endforeach
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <button type="submit"
                                                            class="primary-btn mr-40 fix-gr-bg nowrap submit">
    ...
    <div class="modal fade admin-query" id="showQRSectionModal">
...
@push('script')
    <script>
        $(document).ready(function() {
            $(document).on("click", "#attendance_qrcode_btn", function() {
                var i = 0;
                var formData = Object.fromEntries(new URLSearchParams(window.location.search));
                console.log(formData);
                $.ajax({
                    type: "POST",
                    data: formData,
                    // dataType: "json",
                    url: formData['url'] + "/subject-attendance-qrcode",
                    beforeSend: function() {
                        $('#attendance_qrcode_btn').addClass('pre_loader').removeClass(
                            'loader');
                    },
                    success: function(data) {
                        var modal = $('#showQRSectionModal');
                        console.log(data);
                        modal.html(data);
                        modal.modal('show');
                    },
                    ...
                    complete: function() {
                        i--;
                        if (i <= 0) {
                            $('#attendance_qrcode_btn').removeClass('pre_loader')
                                .addClass('loader');
                        }
                    }
                });
