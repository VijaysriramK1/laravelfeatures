                            ...
                            <div role="tabpanel" class="tab-pane fade  @if ($key == 0) active show @endif"
                                ...
                                <x-table>
                                    <table id="table_id" class="table" cellspacing="0" width="100%">
                                        ...
                                        <tbody>
                                            @php
                                                $results = App\Models\StudentRecord::getSmaStudentTakeOnlineExamParent($record->student_id, $record->id);
                                            @endphp
                                                    ...
                                                    <td>
                                                        ...
                                                        @if ($now >= $endTime)
                                                            @if (moduleStatusCheck('OnlineExam') == true)
                                                                <a class="btn primary-btn" title="Answer Script"
                                                                    href="{{ route('om-parent_answer_script', [@$result_view->online_exam_id, @$result_view->student_id, $record->id]) }}">@lang('exam.answer_script')</a>
                                                            @else
                                                                <a class="btn primary-btn small  fix-gr-bg"
                                                                    data-modal-size="modal-lg" title="Answer Script"
                                                                    href="{{ route('parent_answer_script', [@$result_view->online_exam_id, @$result_view->student_id]) }}">@lang('exam.answer_script')</a>
                                                            @endif
                                                        @else
