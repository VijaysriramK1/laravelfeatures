                ...
                <div class="col-lg-12 student-details up_admin_visitor">
                    <div class="white-box mt-10">
                        ...
                        <div class="tab-content">
                            @foreach ($records as $key => $record)
                                <div role="tabpanel"
                                    class="tab-pane fade mt-60 @if ($key == 0) active show @endif"
                                    id="tab{{ $key }}">
                                    <x-table>
                                                ...
                                                @foreach ($contents as $value)
                                                    <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            @if (moduleStatusCheck('University'))
                                                                @lang('study.all_students_of')
                                                                ({{ @$value->semesterLabel->name . '->' . @$value->unDepartment->name }})
                                                            @else
                                                                ...
                                                                @if (@$value->classes != '' && $value->sections != '')
                                                                    @lang('study.all_students_of')
                                                                    ({{ @$value->classes->class_name . '->' . @$value->sections->section_name }})
                                                                @endif
                                                                @if (@$value->classes != '' && $value->sections == null)
                                                                    @lang('study.all_students_of')
                                                                    ({{ @$value->classes->class_name . '->' . 'All Sections' }})
                                                                @endif
                                                            @endif
                                                        </td>
                                                        ...
                                                        <td>
                                                            <x-drop-down>
                                                                <a data-modal-size="modal-lg" title="View Content Details"
                                                                    class="dropdown-item modalLink"
                                                                    href="{{ route('upload-content-student-view', $value->id) }}">@lang('common.view')</a>
                                                                @if (@$value->upload_file != '')
                                                                    <a class="dropdown-item"
                                                                        href="{{ url(@$value->upload_file) }}" download>
                                                                        @lang('study.download') <span
