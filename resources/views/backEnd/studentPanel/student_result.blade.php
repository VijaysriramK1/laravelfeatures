            ...
            <div class="white-box">
                <ul class="nav nav-tabs tabs_scroll_nav ml-0" role="tablist">
                    ...
                    @foreach ($records as $key => $record)
                        <li class="nav-item">
                            <a class="nav-link @if ($record->is_default == 1) active @endif "
                ...
                <div class="tab-content">
                    @foreach ($records as $key => $record)
                        <div role="tabpanel"
                            class="tab-pane mt-60 fade @if ($record->is_default == 1) active show @endif"
                            id="tab{{ $key }}">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="no-search no-paginate no-table-info mb-2">
                                            @foreach ($exam_terms as $exam)
                                                ...
                                                @endphp
                                                @if ($get_results)
                                                    @if (isset($exam->examSettings->publish_date) && $exam->examSettings->publish_date <= $today)
                                                        <div class="main-title mt-5">
                                                            <h3 class="mb-0">{{ @$exam->title }}</h3>
                                                            <form id="form-{{ @$exam->id }}"
                                                                action="{{ route('student_result_download') }}"
                                                                method="POST" target="_blank">
                                                                <input type="hidden" value="{{ @$exam->id }}"
                                                                    name="exam_id">
                                                                <input type="hidden" value="{{ @$record->class_id }}"
                                                                    name="class_id">
                                                                <input type="hidden" value="{{ @$record->student_id }}"
                                                                    name="student_id">
                                                                <input type="hidden" value="{{ @$record->id }}"
                                                                    name="record_id">
                                                                <input type="hidden" value="{{ @$record->section_id }}"
                                                                    name="section_id">
                                                                @csrf
                                                                <a href="javascript:void(0)"
                                                                    onclick="$('#form-{{ @$exam->id }}').submit()"
                                                                    class="primary-btn small fix-gr-bg pull-right mb-20">
                                                                    <span class="ti-download pr-2"></span>
                                                                    @lang('student.download_marksheet')
                                                                </a>
                                                            </form>
                                                        </div>
                                                        @php
                                                                    ...
                                                                    <table id="" class="table mb-5" cellspacing="0"
                                                                        ...
                                                                        <thead>
                                                                            <tr>
                                                                                ...
                                                                                <th style="text-align: center">@lang('exam.obtained_marks')
                                                                                </th>
                                                                                @if (@generalSetting()->result_type == 'mark')
                                                                                    <th style="text-align: center">
                                                                                        @lang('exam.pass_fail')</th>
                                                                                @else
                                                                                    <th style="text-align: center">
                                                                                        @lang('exam.grade')</th>
                                                                                    <th style="text-align: center">
