            ...
            <tr>
                ...
                <td style="text-aligh:center">
                    <p style="font-size:14px !important; border-bottom:1px solid gray;" align="left"
                        class="text-white">@lang('exam.exam') : {{ @$exam->title }} </p>
                    <p style="font-size:14px !important; border-bottom:1px solid gray" align="left"
                        class="text-white">@lang('common.class'): {{ @$class->class_name }} </p>
                    <p style="font-size:14px !important; border-bottom:1px solid gray" align="left"
                        class="text-white">@lang('common.section'): {{ @$section }} </p>
                    <p style="font-size:14px !important; border-bottom:1px solid gray" align="left"
                        class="text-white">@lang('common.academic_year'): {{ @$academic_year->title }}
                        ({{ @$academic_year->year }}) </p>
        ...
        <table class="table table-bordered table-striped" cellspacing="0" width="100%">
            ...
            <tr>
                ...
                @foreach ($exam_periods as $exam_period)
                    <th style="text-align:center" nowrap>
                        {{ @$exam_period->period }}<br>{{ date('h:i A', strtotime(@$exam_period->start_time)) }}<br>To<br>{{ date('h:i A', strtotime(@$exam_period->end_time)) }}
                    </th>
                @endforeach
            </tr>
            @foreach ($assign_subjects as $assign_subject)
                <tr>
                    ...
                    @if ($section_id == 0)
                        <td>{{ $assign_subject->class->class_name . '(' . $assign_subject->section->section_name . ')' }}
                        </td>
                    @endif
                    @foreach ($exam_periods as $exam_period)
                        @php
