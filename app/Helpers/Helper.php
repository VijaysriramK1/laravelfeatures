...
if (!function_exists('userPermission')) {
    function userPermission($route, $role_id = null, $purpose = null)
    {
        ...
        if ($role_id == 1  || $role_id == 5 && Auth::user()->is_administrator == "yes") {
            return true;
        }

        if ((!empty($permissions)) && ($role_id != 1 || $role_id != 5)) {
    ...
    function moduleStatusCheck($module)
    {
        try {
            ...
            $is_verify = Cache::rememberForever('module_' . $module, function () use ($module) {
                return SmaModuleManager::where('name', $module)->first();
            });
...
if (!function_exists('getUserLanguage')) {
    function getUserLanguage()
    {
        ...
        if (Auth::check()) {
            ...
            $user = User::where('role_id', ['1', '5'])->where('school_id', $school_id)->first();
...
if (!function_exists('menuPosition')) {
    function menuPosition($id)
    {
        ...
        if ($is_have) {
            $sidebar = app('sidebar_news')->where('active_status', 1)->where('sma_module_id', $id)->first();
...
if (!function_exists('menuStatus')) {
    function menuStatus($id)
    {
        $is_have = count(app('sidebar_news')) > 0;
        if (($is_have)) {
            $is_have_id = app('sidebar_news')->where('sma_module_id', $id)->first();
            if ($is_have_id) {
                return $is_have_id->active_status == 1 ? true : false;
            } else {
                return auth()->user()->role_id == 1 || auth()->user()->role_id == 5  ? true : userPermission($id);
            }
        }
        return true;
    }
...
if (!function_exists('student_badge')) {
    function student_badge($student_id)
    {
        $record = StudentRecord::where('active_status', 1)
            ...
            ->with('badge')
            ->first();
        return isset($record->badge) ? asset($record->badge?->image) : asset('public/uploads/badges/default-badge.png');
    }
}
...
if (!function_exists('student_badge')) {
    function my_badge()
    {
        return Cache::remember('student_badge' . auth()->id(), Carbon::now()->endOfDay()->addSecond(), function () {
            $record = StudentRecord::where('active_status', 1)
                ...
                ->with('badge')
                ->first();
            return isset($record->badge) ? asset($record->badge?->image) : null;
        });
    }
}
...
if (!function_exists('get_integration_url')) {
    function get_lead_integration_url()
    {

        $url = '/lead-integration/lead-form';
        return url($url);
    }
}
