...
class MarkRejectedMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'mark_rejected_notification')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Mark Rejected Notification';
        } else {
            $this->mail_subject = 'Mark Rejected Notification';
        }
    }
    ...
    public function content(): Content
    {
        ...
        return new Content(
            view: 'emails.staff.markrejected',
            with: [
                'template' => $content,
