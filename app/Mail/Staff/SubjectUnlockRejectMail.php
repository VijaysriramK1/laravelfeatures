...
class SubjectUnlockRejectMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'subject_unlock_request_rejected')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Subject Unlock Request Rejected';
        } else {
            $this->mail_subject = 'Subject Unlock Request Rejected';
        }
    }
    ...
    public function content(): Content
    {
        ...
        $replacements = [
            ...
            '[school_email]' => $this->smAssignSubject->teacher->email ?? '',
        ];

        $content = strtr($template, $replacements);
        return new Content(
            view: 'emails.staff.subjectunlockreject',
            with: [
                'template' => $content,
