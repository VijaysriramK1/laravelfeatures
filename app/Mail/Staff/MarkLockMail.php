...
class MarkLockMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'mark_lock')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Mark Lock';
        } else {
            $this->mail_subject = 'Mark Lock';
        }
    }
