...
class SubjectUnlockMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'subject_unlocked')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Subject Unlocked';
        } else {
            $this->mail_subject = 'Subject Unlocked';
        }
    }
    ...
    public function content(): Content
    {
        ...
        return new Content(
            view: 'emails.staff.subjectunlock',
            with: [
                'template' => $content,
