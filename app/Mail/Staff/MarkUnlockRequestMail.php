...
class MarkUnlockRequestMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'mark_unlock_request')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Mark Unlock Request';
        } else {
            $this->mail_subject = 'Mark Unlock Request';
        }
    }
    ...
    public function content(): Content
    {
        ...
        return new Content(
            view: 'emails.staff.markunlockrequest',
            with: [
                'template' => $content,
