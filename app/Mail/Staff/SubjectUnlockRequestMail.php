...
class SubjectUnlockRequestMail extends Mailable
{
    ...
    {
        $this->get_mail_template = DB::table('sms_templates')->where('type', 'email')->where('purpose', 'subject_unlock_request')->where('school_id', $smAssignSubject->school_id)->first();

        if (!empty($this->get_mail_template)) {
            $this->mail_subject = $this->get_mail_template->subject ?? 'Subject Unlock Request';
        } else {
            $this->mail_subject = 'Subject Unlock Request';
        }
    }
    ...
    public function content(): Content
    {
        ...
        return new Content(
            view: 'emails.staff.subjectunlockrequest',
            with: [
                'template' => $content,
