...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        try {
            ...
            if ($role_id == 2) {
                #Student
                $get_user_status = Auth::user()->student->active_status;
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $present_students = SmStudent::where('active_status', 1)
                ->where('school_id', $school_id)
                ->whereHas('attendances', function ($q) {
                    $q->where('attendance_date', Carbon::now()->format('Y-m-d'))->where('attendance_type', 'P');
                })
                ->count();

            $leaveStudents = SmStudent::where('active_status', 1)
                ->where('school_id', $school_id)
                ->whereHas('attendances', function ($q) {
                    $q->where('attendance_date', Carbon::now()->format('Y-m-d'))->where('attendance_type', 'A');
                })
                ->count();

            // $present_subject_students = SmStudent::where('active_status', 1)
            //     ->where('school_id', $school_id)
            //     ->whereHas('SubjectWiseAttendances', function ($q) {
            //         $q->where('attendance_date', Carbon::now()->format('Y-m-d'))->where('attendance_type', 'P');
            //     })
            //     ->count();

            $total_classes = SmClass::where('active_status', 1)->count();
            // $total_subjects = SmSubject::where('active_status', 1)->count();
            $days = [
                0 => 2,
                1 => 3,
                2 => 4,
            ...
            $live_classes = SmClass::where('active_status', 1)
                ->where('school_id', $school_id)
                ->whereHas('routineUpdates', function ($q) use ($days) {
                    $q->where('day', $days[Carbon::now()->dayOfWeek])
                        ->where(function ($query) {
                            $query->where('start_time', '<=',  date('H:i:s'))
            ...
            $data = [
                ...
                // 'presentSubjectStudents' => $present_subject_students,
            ];
            if (moduleStatusCheck('Wallet')) {
