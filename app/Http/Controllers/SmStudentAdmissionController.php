...
class SmStudentAdmissionController extends Controller
{
    ...
    public function importPdf(Request $request)
    {
        ...
        try {
            ...
            $current_academic_year = SmAcademicYear::where('school_id', Auth::user()->school_id)
                ->first();

            if (!$current_academic_year) {
                return response()->json(['status' => 'error', 'message' => 'No current academic year set.'], 422);
            }

            $parser = new Parser();
            $pdf = $parser->parseFile($request->file('pdfFile')->getPathname());
            $text = $pdf->getText();

            // Split the text into lines and process each student's data
            $lines = explode("\n", $text);
            $studentData = [];
            $currentStudent = [];
            ...
            foreach ($lines as $line) {
                ...
                // Assuming each student's data is separated by a specific marker
                // Adjust this logic based on your PDF structure
                if (strpos($line, 'STUDENT INFORMATION') !== false) {
                ...
                // Parse each line and add to currentStudent
                // Adjust these based on your PDF structure
                if (preg_match('/^Name:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['full_name'] = trim($matches[1]);
                } elseif (preg_match('/^Admission No:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['admission_number'] = trim($matches[1]);
                }
                // Add more fields as needed...
            }

            // Add the last student if any
            if (!empty($currentStudent)) {
                $studentData[] = $currentStudent;
            }

            foreach ($studentData as $value) {
                // Perform checks (e.g., duplicate admission number, email, etc.)
                // Similar to CSV import...

                // Create student user
                $user_stu = User::create([
                    'role_id' => 2,
                    'full_name' => $value['full_name'],
                    'username' => $value['email'] ?? $value['admission_number'],
                ...
                // Create parent user and parent record
                // Similar to CSV import...
                ...
                SmStudent::create([
                    'class_id' => $request->class,
                    'section_id' => $request->section,
                    'session_id' => $current_academic_year->id,
                    'user_id' => $user_stu->id,
                    'parent_id' => $parent->id ?? null,
                    'role_id' => 2,
                    'admission_no' => $value['admission_number'],
                    'full_name' => $value['full_name'],
                    // Add other fields as available in your PDF...
                    'school_id' => Auth::user()->school_id,
                    'academic_id' => $current_academic_year->id,
                    'created_at' => Carbon::now()->format('Y-m-d H:i:s'),
                ]);
            }

            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Students imported successfully from PDF']);
        } catch (\Exception $e) {
            DB::rollback();
            return response()->json(['status' => 'error', 'message' => 'Operation Failed: ' . $e->getMessage()], 500);
        }
    }
    ...
    public function importCsv(Request $request)
    {
        ...
        try {
            ...
            Excel::import(new StudentsImport, $request->file('csvFile'));
            $data = StudentBulkTemporary::where('user_id', Auth::user()->id)->get();
            ...
            if (!empty($data)) {
                foreach ($data as $value) {
                    ...
                    $user_parent = User::create([
                        'role_id' => 3,
                        'full_name' => $value->father_name,
                        'username' => $value->guardian_email ?? 'par_' . $value->admission_number,
                    ...
                    // Create parent
                    $parent = SmParent::create([
                        'user_id' => $user_parent->id,
                        'fathers_name' => $value->father_name,
                        'fathers_mobile' => $value->father_phone,
                        'fathers_occupation' => $value->fathe_occupation,
                        'mothers_name' => $value->mother_name,
                        'mothers_mobile' => $value->mother_phone,
                        'mothers_occupation' => $value->mother_occupation,
                        'guardians_name' => $value->guardian_name,
                        'guardians_mobile' => $value->guardian_phone,
                        'guardians_occupation' => $value->guardian_occupation,
                        'guardians_address' => $value->guardian_address,
                        'guardians_email' => $value->guardian_email,
                        'school_id' => Auth::user()->school_id,
                        'academic_id' => $current_academic_year->id,
                        'created_at' => Carbon::now()->format('Y-m-d H:i:s'),
                    ]);
                    ...
                    SmStudent::create([
                        ...
                        'bloodgroup_id' => $value->blood_group,
                        'religion_id' => $value->religion,
                        'height' => $value->height,
                        'weight' => $value->weight,
                        'current_address' => $value->current_address,
                        'permanent_address' => $value->permanent_address,
                        'national_id_no' => $value->national_identification_no,
                        'local_id_no' => $value->local_identification_no,
                        'bank_account_no' => $value->bank_account_no,
                        'bank_name' => $value->bank_name,
                        'previous_school_details' => $value->previous_school_details,
                        'aditional_notes' => $value->note,
                        'school_id' => Auth::user()->school_id,
                        'academic_id' => $current_academic_year->id,
                        'created_at' => Carbon::now()->format('Y-m-d H:i:s'),
                    ]);
                ...
                return response()->json(['status' => 'error', 'message' => 'No data found in the CSV file'], 422);
            }
        } catch (\Exception $e) {
            DB::rollback();
            return response()->json(['status' => 'error', 'message' => 'Operation Failed: ' . $e->getMessage()], 500);
    ...
    public function graduatedStudent(Request $request)
    {
        try {

            $pt = trans('student.graduated_students');
            $students = SmStudent::where('school_id', Auth::user()->school_id)
                // ->where('active_status', 1)
                ->whereHas('allRecords')
                ->whereDoesntHave('allRecords', function ($query) {
                    $query->where('is_promote', 0);
                })
                ->get();
            ...
            return view('backEnd.studentInformation.graduated_students', compact('students', 'classes', 'pt'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function graduatedStudentSearch(Request $request)
    {
        try {
            $pt = trans('student.graduated_students');
            $student_ids = StudentRecord::when($request->class, function ($query) use ($request) {
            ...
            $students = SmStudent::query()->withOutGlobalScope(StatusAcademicSchoolScope::class);
            $students->where('academic_id', getAcademicId())
                // ->where('active_status', 1)
                ->whereHas('allRecords')->whereDoesntHave('allRecords', function ($query) {
                    $query->where('is_promote', 0);
                });
            if ($request->name != "") {
                $students->where('full_name', 'like', '%' . $request->name . '%');
            ...
            return view('backEnd.studentInformation.graduated_students', compact('classes', 'class_id', 'section_id', 'name', 'admission_no', 'pt'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    {
        return view('backEnd.studentInformation.student_unapproved');
    }
}
