...
class SmStudentPromoteController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            if ($generalSetting->promotionSetting == 0) {
                ...
                return view('backEnd.studentInformation.student_promote_new', compact('sessions', 'classes', 'badges'));
            } else {
    ...
    public function promote(Request $request)
    {
        ...
        $validationRules = [
            ...
            'promote.*.badge' => $isGraduateSelected && generalSetting()->badge_system == 0 ? 'nullable' : 'required',
        ];

        if (generalSetting()->badge_system == 1) {
            $validationRules['promote.*.badge'] =  $isGraduateSelected  ? 'nullable' : 'required';
        }
                ...
                if (gv($student_data, 'student') && gv($student_data, 'class') && gv($student_data, 'section')) {
                    ...
                    if (!$exit_record) {
                        ...
                        $result = $insertStudentRecord->insertStudentRecord($request->merge([
                            ...
                            'section' => gv($student_data, 'section'),
                            'session' => $request->promote_session,
                            'badge_id' => gv($student_data, 'badge', null),
                        ]));
