...
class DatatableQueryController extends Controller
{
    public function studentDetailsDatatable(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $all_students =  SmStudent::with('studentRecords', 'studentRecords.class', 'studentRecords.section')
            ...
            return Datatables::of($all_students)
                ...
                ->addColumn('action', function ($row) {
                    ...
                    $btn = '<div class="dropdown CRM_dropdown">
                        ...
                        . '<a class="dropdown-item viewclick" href="' . route('student_edit', [$row->id]) . '">' . app('translator')->get('common.view') . '</a>' .
                        (userPermission('student_edit') === true ? '<a class="dropdown-item editclick" href="' . route('student_edit', [$row->id]) . '">' . app('translator')->get('common.edit') . '</a>' : '') .
                        ...
                        (userPermission('disabled_student') === true ? (Config::get('app.app_sync') ? '<span  data-toggle="tooltip" title="Disabled For Demo "><a  class="dropdown-item" href="#"  >' . app('translator')->get('common.disable') . '</a></span>' :
                            ...
                            <a type="button" class="dropdown-item ' . $status . '" data-id="' . $row->id . '"><i class="bi bi-pencil"></i><span class="ms-2">' . $label_name . '</span></a>
                            
                            </div>
                                </div>';

                    return $btn;
                })->addColumn('student_badge', function ($row) {
                    $student_badge = '<img width="32" height="32" style="border-radius:50%" src="' . student_badge($row->id) . '" />';
                    return student_badge($row->id) ? $student_badge : null;
                })
                ->rawColumns(['action', 'student_badge', 'full_name', 'mobile', 'dob', 'class_sec', 'full_name', 'mobile', 'dob', 'class_sec'])
                ->make(true);
        }
            ...
            if (Auth::user()->role_id == 1) {
                ...
                $staffs->withOutGlobalScope(ActiveStatusSchoolScope::class)->where('school_id', Auth::user()->school_id)
                    ...
                    ->with(['roles' => function ($query) {
                        $query->whereNotIn('id', [2, 3, 5, 6, 7, 8, 9])
                            ->select('id', 'name');
                    }])
    ...
    {
        if ($request->ajax()) {
            $all_students = SmStudent::with('parents', 'gender', 'category')
    ...
    public function graduatedStudentList(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $all_students = $students->with('studentRecords.class', 'parents', 'gender', 'category')->where('school_id', Auth::user()->school_id);
            ...
            return Datatables::of($all_students)
                ...
                ->addColumn('semester_label', function ($row) use ($request) {
                    $semester_label = [];
                    foreach ($row->allRecords as $label) {
                        if (moduleStatusCheck('University')) {
                            $semester_label[] = $label->unSemesterLabel->name;
                        }
                    }
                ...
                ->addColumn('class_sec', function ($row) use ($request) {
                    $class_sec = [];
                    foreach ($row->allRecords as $classSec) {
                        if (moduleStatusCheck('University')) {
                            $class_sec[] = $classSec->unFaculty->name . '(' . $classSec->unDepartment->name . ')';
                        } else {
                            $class_sec[] = $classSec->class->class_name . '(' . $classSec->section->section_name . ')';
