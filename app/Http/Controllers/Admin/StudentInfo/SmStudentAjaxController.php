...
class SmStudentAjaxController extends Controller
{
    ...
    public function ajaxStudentPromoteGroup(Request $request)
    {
        $promote_sections = [];
    
        if ($request->parent) {
            $sectionIds = SmClassGroup::where('class_id', $request->id)
                ->where('school_id', Auth::user()->school_id)->get();
    
            foreach ($sectionIds as $sectionId) {
                $group = SmStudentGroup::where('id', $sectionId->group_id)
                    ->withoutGlobalScope(StatusAcademicSchoolScope::class)
                    ->withoutGlobalScope(GlobalAcademicScope::class)
                    ->whereNull('parent_id')
                    ->first(['id', 'group']); // Ensure field name matches database
    
                if ($group) {
                    $promote_sections[] = $group;
                }
            }
        } else {
            $class = SmClass::find($request->id);
    ...
    {
        // Get the student IDs based on class, section, group, and category
        $student_ids = SmStudentReportController::classSectionStudent($request);
    
        // Query the students with their related parents, based on the student IDs
        $students = SmStudent::with('parents')
            ->whereIn('id', $student_ids)  // Ensure the query is filtered by student IDs
            ->where('active_status', 1)
            ->where('school_id', Auth::user()->school_id)
            ->when($request->group, function ($query, $group) {
                // Filter by group category (group_id) if provided
                return $query->where('group_id', $group);  // group_id refers to group category
            })
            ->when($request->student_group, function ($query, $student_group) {
                // Filter by student group (student_group_id) if provided
                return $query->where('student_group_id', $student_group);  // student_group_id refers to the student's group
            })
            ->get()
