...
class SmStudentAdmissionController extends Controller
{
    ...
    public function view(Request $request, $id, $type = null)
    {
        try {
            ...
            $badges = Badge::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
            ...
            if (moduleStatusCheck('University')) {
                ...
                return view('backEnd.studentInformation.student_view', compact('timelines', 'student_detail', 'driver_info', 'exams', 'siblings', 'grades', 'academic_year', 'exam_terms', 'max_gpa', 'fail_gpa_name', 'custom_field_values', 'sessions', 'records', 'next_labels', 'type', 'studentRecordDetails', 'studentDetails', 'studentRecords', 'result_setting', 'assinged_exam_types', 'studentBehaviourRecords', 'behaviourRecordSetting', 'badges'));
            } else {
                return view('backEnd.studentInformation.student_view', compact('timelines', 'student_detail', 'driver_info', 'exams', 'siblings', 'grades', 'academic_year', 'exam_terms', 'max_gpa', 'fail_gpa_name', 'custom_field_values', 'sessions', 'records', 'next_labels', 'type', 'result_setting', 'attendance', 'subjectAttendance', 'days', 'year', 'month', 'studentBehaviourRecords', 'behaviourRecordSetting', 'badges'));
            }
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
    ...
    public function insertStudentRecord($request, $pre_record = null)
    {
        ...
        if ($request->has('badge_id')) {
            $studentRecord->badge_id = $request->badge_id;
        }
          $studentRecord->group_id = $request->student_group_id;
          $studentRecord->student_category_id = $request->student_category_id;
        $studentRecord->save();
    ...
    public function assignClass($id)
    {
        ...
        $data['badges'] = Badge::where('active_status', 1)->where('school_id', Auth::user()->school_id)->get();
                    ...
                    foreach ($data as $key => $value) {
                        ...
                        if ($ad_check->count() > 0) {
                            ...
                            if ($value->admission_number != '') {
                                $exists = SmStudent::where('admission_no', $value->admission_number)
                                    ->where('school_id', Auth::user()->school_id)
                                    ->exists();
                                if ($exists) {
                        ...
                        try {
                            ...
                            $user_stu = new User();
                            $user_stu->role_id = 2;
                            $user_stu->full_name = $value->full_name;
    ...
    public function importPdf(Request $request)
    {
        ...
        try {
            ...
            foreach ($lines as $line) {
                ...
                if (preg_match('/^Admission No\s*:\s*(.*)$/i', $line, $matches)) {
                    if (!empty($currentStudent)) {
                        $students[] = $currentStudent;
                        $currentStudent = [];
                    }
                    $currentStudent['admission_no'] = trim($matches[1]);
                } elseif (preg_match('/^Name\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['name'] = trim($matches[1]);
                } elseif (preg_match('/^Father Name\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['father_name'] = trim($matches[1]);
                } elseif (preg_match('/^Status\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['status'] = trim($matches[1]);
                } elseif (preg_match('/^Date Of Birth\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['dob'] = trim($matches[1]);
                } elseif (preg_match('/^Class\(Section\)\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['class_section'] = trim($matches[1]);
                } elseif (preg_match('/^Gender\s*:\s*(.*)$/i', $line, $matches)) {
                    $currentStudent['gender'] = trim($matches[1]);
                }
            }
            ...
            foreach ($students as $studentData) {
                $student = SmStudent::updateOrCreate(
                    ['admission_no' => $studentData['admission_no']],
                    [
                        'name' => $studentData['name'],
                        'father_name' => $studentData['father_name'],
                        'status' => $studentData['status'],
                        'date_of_birth' => Carbon::parse($studentData['dob'])->format('Y-m-d'),
            ...
            return response()->json(['status' => 'success', 'message' => count($students) . ' students imported/updated successfully']);
        } catch (\Exception $e) {
