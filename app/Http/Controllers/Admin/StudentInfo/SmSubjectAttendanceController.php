...
class SmSubjectAttendanceController extends Controller
{
    ...
    {
        try {
            $input['date'] = date('Y-m-d', strtotime($request->attendance_date));
            $input['class'] = $request->class_id;
            $input['subject'] = $request->subject_id;
            $input['section'] = $request->section_id;
            $code = AttendanceCode::where('date', $input['date'])
                ->where('class_id', $input['class'])
                ->where('section_id', $input['section'])
            ...
            if (!isset($code)) {
                ...
                $code->class_id = $input['class'];
                $code->code = Uuid::uuid1();
                $code->subject_id = $input['subject'];
                $code->section_id = $input['section'];
                $code->date = $input['date'];
                $code->type = 'subject';
                $code->created_by = auth::id();
                $code->start_at = $request->start_at ?: null;
                $code->end_at = $request->end_at ?: null;
                $code->save();
            }

            $qrcode = $code->code;
            return view('backEnd.studentInformation.inc._attendance_qrcode', compact('qrcode', 'input'));
        } catch (\Exception $e) {
            // Toastr::error('Operation Failed', 'Failed');
            // return redirect()->back();
