...
class SmOnlineExamController extends Controller
{
    ...
    public function onlineExamReport(Request $request)
    {
        try {
            if (moduleStatusCheck('OnlineExam') == TRUE) {
                $exams = SmaOnlineExam::where('active_status', 1)
                    ->where('academic_id', getAcademicId())
                    ->where('school_id', Auth::user()->school_id)
                    ->get();
            } else {
    ...
    function universityOnlineExamReportSearch($request)
    {
        ...
        try {
            ...
            if (moduleStatusCheck('OnlineExam') == TRUE) {
                $online_exam_question = SmaOnlineExam::find($request->exam);
                $online_exam = SmaOnlineExam::where('class_id', $request->class)->where('section_id', $request->section)->where('id', $request->exam)->where('end_date_time', '<', $present_date_time)->where('status', 1)->first();
                $present_students = [];
                foreach ($students as $student) {
                    $take_exam = SmaStudentTakeOnlineExam::where('student_id', $student->id)->where('online_exam_id', $online_exam_question->id)->first();
                    if ($take_exam != "") {
                        $present_students[] = $student->id;
                    }
                }
                ...
                $exams = SmaOnlineExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            } else {
