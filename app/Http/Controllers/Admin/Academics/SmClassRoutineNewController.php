...
class SmClassRoutineNewController extends Controller
{
    ...
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            if ($subjects->isNotEmpty()) {
                $teachers = SmStaff::whereIn('id', $subjects->pluck('teacher_id'))->where('role_id', 4)->where('school_id', Auth::user()->school_id)->get();
            } else {
                $teachers = '';
            }
    ...
    public function dayWiseClassRoutine(Request $request)
    {
        ...
        $check_subjects = SmAssignSubject::where('class_id', $class_id)
            ->where('section_id', $section_id)
            ->where('school_id', Auth::user()->school_id)
            ->get();

        if ($check_subjects->isNotEmpty()) {
            $teachers = SmStaff::whereIn('id', $check_subjects->pluck('teacher_id'))->where('role_id', 4)->where('school_id', Auth::user()->school_id)->get();
        } else {
            $teachers = '';
        }
    ...
    public function addNewClassRoutineStore(Request $request)
    {
        try {
            ...
            foreach ($request->routine as $key => $routine_data) {
                if ((!gbv($routine_data, 'is_break') && !gv($routine_data, 'subject')) || !gv($routine_data, 'start_time') || !gv($routine_data, 'end_time') || !gv($routine_data, 'start_date') || !gv($routine_data, 'end_date')) {
                    continue;
                }
                ...
                foreach ($days as $day) {
                    $exist_class_routine = SmClassRoutineUpdate::where('day', $day)
                        ...
                        ->where('academic_id', getAcademicId())
                        ->where('school_id', Auth::user()->school_id)
                        ->where('start_date', gv($routine_data, 'start_date'))
                        ->where('end_date', gv($routine_data, 'end_date'))
                        ->first();
                    ...
                    $class_routine->start_date = gv($routine_data, 'start_date');
                    $class_routine->end_date = gv($routine_data, 'end_date');
                    $class_routine->save();
                }
            }
