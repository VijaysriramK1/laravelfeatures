...
class SmPayrollController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            $data['roles'] = SmaRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q) {
    ...
    public function searchStaffPayr(Request $request)
    {
        ...
        try {
            ...
            $roles = SmaRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q) {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
    ...
    public function savePayrollPaymentData(Request $request)
    {
        ...
        try {
            ...
            $data['roles'] = SmaRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q) {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
