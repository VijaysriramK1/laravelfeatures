...
class SmStaffController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->SmaModuleManager = json_encode(SmaModuleManager::find(1));
        $this->URL = url('/');
    }
    ...
    public function staffList(Request $request)
    {
        try {
                $roles = SmaRole::query();
                $roles->whereNotIn('id', [2, 3, 5, 6, 7, 8, 9]);
    ...
    public function addStaff()
    {
        ...
        try {
            $max_staff_no = SmStaff::withOutGlobalScope(ActiveStatusSchoolScope::class)->where('is_saas', 0)
                ...
                $roles = SmaRole::where('is_saas', 0)
                     ->where('active_status', 1)
                     ->where(function ($q) {
                          $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                     })
                     ->whereNotIn('id', [2, 3, 5, 6, 7, 8, 9])
                     ->get()
                     ->sortBy(function ($role) {
                       if ($role->id == 1) {
                           return 1;
    ...
    public function staffStore(staffRequest $request)
    {
        ...
        try {
            ...
            try {
                ...
                $user->is_administrator = $request->role_id == 1 ? 'yes' : 'no';
    ...
    public function searchStaff(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()->role_id != 1) {
                $roles = SmaRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 5)->where(function ($q) {
                    $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                })->get();
            } else {
                $roles = SmaRole::where('is_saas', 0)->where('active_status', '=', '1')->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q) {
                    $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                })->get();
            }
