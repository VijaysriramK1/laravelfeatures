...
class RouteListController extends Controller
{
  ...
  public function index()
  {
    ...
    $zoom = SmaModuleInfo::where('name', 'like', '%' . 'zoom' . '%')->orWhere('route', 'like', '%' . 'zoom' . '%')->update(['module_name' => 'Zoom']);
    $saas = SmaModuleInfo::where('name', 'like', '%' . 'saas' . '%')->orWhere('route', 'like', '%' . 'saas' . '%')->update(['module_name' => 'Saas']);
    $bbb = SmaModuleInfo::where('name', 'like', '%' . 'bbb' . '%')
      ->orWhere('route', 'like', '%' . 'bbb' . '%')
      ->orWhere('route', 'like', '%' . 'bigbluebutton' . '%')
      ->update(['module_name' => 'BBB']);
    $jitsi = SmaModuleInfo::where('name', 'like', '%' . 'jitsi' . '%')->orWhere('route', 'like', '%' . 'jitsi' . '%')->update(['module_name' => 'Jitsi']);
    $parentregistration = SmaModuleInfo::where('name', 'like', '%' . 'parentregistration' . '%')->orWhere('route', 'like', '%' . 'parentregistration' . '%')->update(['module_name' => 'ParentRegistration']);

    $formatRouteList = $this->routeFormat();
    foreach ($formatRouteList as $key => $list) {
      $va = SmaModuleInfo::updateOrCreate(
        [
          'id' => $key
        ],
        [
  ...
  public function update()
  {
    ...
    $all = SmaModuleInfo::get();
    $routes =  SmaModuleInfo::where('parent_id', 0)->get();
    $modifiyList = [];
    foreach ($all as $item) {
      // $parent_route = SmaModuleInfo::where('parent_id', $item->id)->where('parent_id', '!=',0)->update(['parent_route'=>$item->route]);
      // $modifiyList[]=[
      //    'id'=>$item->id,
      //    'type'=>$item->type,
      //    'route'=>$item->route,
    ...
    $routes =  SmaModuleInfo::get(['id', 'name', 'route', 'parent_route', 'type']);
    $formatRoute = [];
  ...
  private function routeFormat()
  {
    return $formatRouteList = array(
      ...
      array(
        'name' => 'Add',
        'route' => 'fees.fees-invoice-post',
        'parent_route' => 'fees.fees-post',
        'type' => 3,
      ),
      15211 =>
      array(
