...
use Modules\OnlineExam\Entities\SmaWrittenExam;
...
use Modules\OnlineExam\Entities\SmaStudentTakeOnlineExam;
...
class StudentRecord extends Model
{
   ...
   public function Subjects(){
    return $this->belongsTo(SmAssignSubject::class,'class_id','class_id');
   }
    ...
    public function getOnlineExamAttribute()
    {
        ...
        if (moduleStatusCheck('OnlineExam') == true) {
            if (moduleStatusCheck('University')) {
                return SmaOnlineExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('status', 1)
            ...
            return SmaOnlineExam::with('studentSubmitExamWithStatus')->where('active_status', 1)->where('academic_id', getAcademicId())->where('status', 1)->where('class_id', $this->class_id)
                ->where('school_id', Auth::user()->school_id)
    ...
    public function getSmaStudentTakeOnlineExamAttribute()
    {
        if (moduleStatusCheck('OnlineExam') == true && auth()->user()->role_id == 2) {
            return  SmaStudentTakeOnlineExam::where('status', 2)
    ...
    public static function getSmaStudentTakeOnlineExamParent($student_id, $record_id)
    {
        if (moduleStatusCheck('OnlineExam') == true) {
            return SmaStudentTakeOnlineExam::where('status', 2)
    ...
    {
        if (moduleStatusCheck('University')) {
            return SmaWrittenExam::where('active_status', 1)->where('un_academic_id', getAcademicId())->where('status', 1)->where('un_semester_label_id', $this->un_semester_label_id)->where('school_id', Auth::user()->school_id)->get();
        } else {
            return SmaWrittenExam::where('active_status', 1)->where('academic_id', getAcademicId())->where('status', 1)->where('class_id', $this->class_id)->where('section_id', $this->section_id)->where('school_id', Auth::user()->school_id)->with('class', 'section', 'subject')->get();
        }
    }
