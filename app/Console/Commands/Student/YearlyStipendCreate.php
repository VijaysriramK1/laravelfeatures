...
class YearlyStipendCreate extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'student:yearly-stipend-create';
     ...
     * @var string
     */
    protected $description = 'Student yearly stipend create';
    ...
    {
        $check_stipend = Stipend::whereYear('start_date', '<=', Carbon::now()->year)->where('interval_type', 'yearly')->get();

        if ($check_stipend->isNotEmpty()) {
            $check_stipend_payment = StipendPayment::whereIn('stipend_id', $check_stipend->pluck('id'))->get();
            $current_date = Carbon::now()->format('Y-m-d');
            $first_month_and_date_of_this_year = Carbon::now()->firstOfYear()->startOfMonth()->toDateString();

            if ($current_date == $first_month_and_date_of_this_year) {
