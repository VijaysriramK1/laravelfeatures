...
use Modules\OnlineExam\Entities\SmaPdfExam;
...
use Modules\FeesCollection\Entities\SmaFeesMaster;
use Modules\BehaviourRecords\Entities\AssignIncident;
use Modules\FeesCollection\Entities\SmaFeesPayment;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Modules\FeesCollection\Entities\SmaAssignDiscount;
use Modules\OnlineExam\Entities\SmaStudentTakeOnlineExam;
use Modules\OnlineExam\Entities\SmaStudentTakeWrittenExam;
...
class SmStudent extends Model
{
    ...
    public function studentCategorys()
    {
        return $this->belongsTo('App\SmStudentCategory', 'student_category_id', 'id')->withDefault();
    }
    ...
    {
        return $this->hasMany(SmaAssignDiscount::class, 'student_id');
    }

    public function feesMasters()
    {
        return $this->hasMany(SmaFeesMaster::class, 'class_id', 'class_id');
    }
    ...
    public function studentOnlineExams()
    {
        ...
        if (moduleStatusCheck('OnlineExam') == true) {
            return $this->hasMany(SmaOnlineExam::class, 'class_id', 'class_id')->where('section_id', $this->section_id)
    ...
    {
        return $this->hasMany(SmaPdfExam::class, 'class_id', 'class_id')->where('section_id', $this->section_id)
            ->where('active_status', 1)->where('status', 1)->where('school_id', Auth::user()->school_id);
    }
    ...
    {
        if (moduleStatusCheck('OnlineExam') == true) {
            return $this->hasMany(SmaStudentTakeOnlineExam::class, 'student_id');
        } else {
    ...
    {
        if (moduleStatusCheck('OnlineExam') == true) {
            return $this->hasMany(SmaStudentTakeWrittenExam::class, 'student_id');
        }
    }
    ...
    {
        return $this->hasMany(SmaFeesPayment::class, 'student_id');
    }
