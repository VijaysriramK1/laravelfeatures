...
trait SidebarDataStore
{
    ...
    { {
            ...
            if (count($oldSidebar) > 0) {
                $oldPermissionIds = $oldSidebar->pluck('sma_module_id')->toArray();
    ...
    function resetSidebarStore()
    {
        ...
        $administration_sections = ["admin_section", "academics", "study_material", "program",'download-center', "lesson-plan", "bulk_print", "certificate", "university", "lms"];
        $student_sections = ["student_info", "fees","scholarships","fees_collection", "transport", "dormitory", "library", "homework", "behaviour_records", "alumni_records"];
        ...
        foreach ($userPermissionSections as $key => $userSection) {
            $parent = $userSection->parent_route != null
                ...
                ->when(auth()->user()->role_id == 2 || auth()->user()->role_id == 10, function ($q) {
    ...
    function permissions()
    {
        ...
        if ($user->role_id == 1) {
            ...
            $permissionInfos = Permission::where('is_menu', 1)
                ...
                ->when(!in_array($user->role_id, [2, 3, 10]), function ($q) {
                    $q->where('is_admin', 1);
                    ...
                    $q->where('user_id', auth()->user()->id)->orWhereNull('user_id');
                })->when($user->role_id == 10, function ($q) {
                    $q->where('is_student', 1);
                })
    ...
    {
        $activeModules = [];
        $modules = SmaModuleManager::whereNotNull('purchase_code')->where('is_default', false)->where('name', '!=', 'OnlineExam')->pluck('name')->toArray();
