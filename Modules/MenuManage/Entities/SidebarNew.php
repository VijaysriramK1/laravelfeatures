...
class SidebarNew extends Model
{
    ...
    {
        $check = SmaPermissionAssign::where('role_id', auth()->user()->role_id)->get('module_id');
        ...
        if (moduleStatusCheck('Saas') == True) {
            ...
            $submenu->whereNotIn('sma_module_id', $ids);
        }
        if (auth()->user()->role_id != 1) {
            $submenu->whereIn('sma_module_id', $check);
            $submenu->whereNotIn('sma_module_id', [833, 834]);
        }
        if (auth()->user()->role_id == 4) {
            $submenu->whereNotIn('sma_module_id', [833, 834]);
        }
        if (auth()->user()->role_id == 1) {
            $submenu->whereNotIn('sma_module_id', [193]);
        }
    ...
    public static function studentMenuDefualt($id)
    {
        ...
        $check = SmaPermissionAssign::where('role_id', auth()->user()->role_id)->get('module_id');
        return SmaModuleStudentParentInfo::where('parent_id', $id)
            ->whereIn('id', $check)
