...
@section('mainContent')
    ...
    <div class="row">
        <div class="col-lg-12 d-flex align-items-center gap_5 justify-content-between flex-wrap ">
            ...
            <a href="{{ route('menumanage.reset') }}" class="primary-btn fix-gr-bg small pull-right"> <i class="ti-reload">
                </i> @lang('menumanage::menuManage.reset')</a>
        </div>
    </div>
    ...
    <div class="erp_role_permission_area ">
        ...
        <div class="mesonary_role_header" id="sortable">
            @foreach ($all_modules as $key => $module_info)
                @if ($check_sidebar != '')
                    @if (auth()->user()->role_id == 1 || in_array($module_info->sma_module_id, $permission_ids))
                        <div class="single_role_blocks">
                            <div class="single_permission" id="{{ $module_info->sma_module_id }}">
                                <div class="permission_header d-flex align-items-center justify-content-between">
                                    <div>
                                        <input type="hidden" name="all_modules_id[]"
                                            value="{{ $module_info->sma_module_id }}">
                                        <input type="checkbox" name="module_id[]" value="{{ $module_info->sma_module_id }}"
                                            id="Main_Module_{{ $key }}"
                                            class="common-radio permission-checkAll main_module_id_{{ $module_info->sma_module_id }}"
                                            {{ $check_sidebar != '' ? (in_array($module_info->sma_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                        <label
                                            for="Main_Module_{{ $key }}">{{ __('menumanage::menuManage.' . $module_info->name) }}</label>
                                    </div>
                                    <div class="arrow collapsed" data-toggle="collapse"
                                        data-target="#Role{{ $module_info->sma_module_id }}"></div>
                                </div>
                                <div id="Role{{ $module_info->sma_module_id }}" class="collapse">
                                    <div class="permission_body">
                                        <ul class="submenuSort">
                                            ...
                                            @foreach ($subModule as $row2)
                                                @if (moduleStatusCheck('Saas') == true && $row2->sma_module_id == 547)
                                                @else
                                                    <li>

                                                        <div class="submodule">
                                                            <input type="hidden" name="all_modules_id[]"
                                                                value="{{ $row2->sma_module_id }}">
                                                            <input id="Sub_Module_{{ $row2->sma_module_id }}"
                                                                name="module_id[]" value="{{ $row2->sma_module_id }}"
                                                                class="sma_csk common-radio module_id_{{ $module_info->sma_module_id }} module_link"
                                                                type="checkbox"
                                                                {{ $check_sidebar != '' ? (in_array($row2->sma_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                            <label
                                                                for="Sub_Module_{{ $row2->sma_module_id }}">{{ __('menumanage::menuManage.' . $row2->name) }}</label>
                                                            <br>
                                                        </div>

                                                        <ul class="option row3">
                                                            <?php
                                                            $childModule = DB::table('sidebar_news')
                                                                ->where('user_id', auth()->user()->id)
                                                                ->where('role_id', auth()->user()->role_id)
                                                                ->where('route', '!=', '')
                                                                ->where('parent_id', $row2->sma_module_id)
                                                                ->orderby('id', 'ASC')
                                                                ->get();
                                                            ?>
                                                            @foreach ($childModule as $row3)
                                                                <li>
                                                                    <div class="module_link_option_div"
                                                                        id="{{ $row2->sma_module_id }}">
                                                                        <input type="hidden" name="all_modules_id[]"
                                                                            value="{{ $row3->sma_module_id }}">
                                                                        <input id="Option_{{ $row3->id }}"
                                                                            name="module_id[]"
                                                                            value="{{ $row3->sma_module_id }}"
                                                                            class="sma_csk common-radio module_id_{{ $module_info->sma_module_id }} module_option_{{ $module_info->sma_module_id }}_{{ $row2->sma_module_id }} module_link_option"
                                                                            type="checkbox"
                                                                            {{ !empty($check_sidebar) ? (in_array($row3->sma_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                                        <label class="nowrap"
                                                                            for="Option_{{ $row3->id }}">{{ __('menumanage::menuManage.' . $row3->name) }}</label>
                                                                        <br>
                                                                    </div>
                                                                </li>
                                                            @endforeach
                        ...
                        <div class="single_permission" id="{{ $module_info->id }}">
                            <div class="permission_header d-flex align-items-center justify-content-between">
                                <div>
                                    ...
                                    <input type="checkbox" name="module_id[]" value="{{ $module_info->id }}"
                                        id="Main_Module_{{ $key }}"
                                        class="common-radio permission-checkAll main_module_id_{{ $module_info->id }}"
                                        {{ $check_sidebar != '' ? (in_array($module_info->id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                    <label
                                        for="Main_Module_{{ $key }}">{{ __('menumanage::menuManage.' . $module_info->name) }}</label>
                                </div>
                                <div class="arrow collapsed" data-toggle="collapse"
                                    data-target="#Role{{ $module_info->id }}"></div>
                            </div>
                                                ...
                                                <li>
                                                    ...
                                                    <div class="submodule">
                                                        ...
                                                        <input id="Sub_Module_{{ $row2->id }}" name="module_id[]"
                                                            value="{{ $row2->id }}"
                                                            class="sma_csk common-radio module_id_{{ $module_info->id }} module_link"
                                                            type="checkbox"
                                                            {{ $check_sidebar != '' ? (in_array($row2->id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                        <label
                                                            for="Sub_Module_{{ $row2->id }}">{{ __('menumanage::menuManage.' . $row2->name) }}</label>
                                                        <br>
                                                    </div>

                                                    <ul class="option row3">
                                                        <?php
                                                        $childModule = DB::table('sma_module_infos')
                                                            ->where('route', '!=', '')
                                                            ...
                                                            <li>
                                                                <div class="module_link_option_div"
                                                                    ...
                                                                    <input type="hidden" name="all_modules_id[]"
                                                                        value="{{ $row3->id }}">
                                                                    <input id="Option_{{ $row3->id }}"
                                                                        name="module_id[]" value="{{ $row3->id }}"
                                                                        class="sma_csk common-radio module_id_{{ $module_info->id }} module_option_{{ $module_info->id }}_{{ $row2->id }} module_link_option"
                                                                        type="checkbox"
                                                                        {{ !empty($check_sidebar) ? (in_array($row3->id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                                                    <label class="nowrap"
                                                                        for="Option_{{ $row3->id }}">{{ $row3->name }}</label>
                                                                    <br>
                                                                </div>
                                                            </li>
                                                        @endforeach
                            ...
                            <div class="single_permission" id="{{ $module_info->id }}">
                                <div class="permission_header d-flex align-items-center justify-content-between">
                                    <div>
                                        <input type="hidden" name="all_modules_id[]" value="{{ $module_info->id }}">
                                        <input type="checkbox" name="module_id[]" value="{{ $module_info->id }}"
                                            id="Main_Module_{{ 10000 - $key }}"
                                            class="common-radio permission-checkAll main_module_id_{{ $module_info->id }}"
                                            {{ $check_sidebar != '' ? (in_array($module_info->id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                        <label
                                            for="Main_Module_{{ 10000 - $key }}">{{ __('menumanage::menuManage.' . $module_info->name) }}</label>
                                    </div>
                                    <div class="arrow collapsed" data-toggle="collapse"
                                        data-target="#Role{{ $module_info->id }}"></div>
                                </div>
            ...
            <div class="col-lg-12 text-center">
                ...
                @if (Illuminate\Support\Facades\Config::get('app.app_sync'))
                    <span class="d-inline-block" tabindex="0" data-toggle="tooltip" title="Disabled For Demo ">
                        <button class="primary-btn fix-gr-bg  demo_view" style="pointer-events: none;"
                            type="button">@lang('menumanage::menuManage.submit') </button></span>
                @else
