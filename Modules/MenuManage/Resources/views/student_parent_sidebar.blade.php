        ...
        @if (Auth::user()->role_id == 2 || Auth::user()->role_id == 3)
            @foreach ($student_parent_menu as $key => $student_parent_module)
                @if (!$student_parent_module->admin_section || isMenuAllowToShow($student_parent_module->admin_section))
                    @if ($check_sidebar != '')
                        @if (in_array($student_parent_module->sma_module_id, $permission_ids))
                            <?php
                            ...
                            <div class="single_role_blocks" data-id="menu_org{{ $student_parent_module->module_id }}">
                                <div class="single_permission" id="{{ $student_parent_module->sma_module_id }}">
                                    <div class="permission_header d-flex align-items-center justify-content-between">
                                        <div>
                                            <input type="checkbox" name="module_id[]"
                                                value="{{ $student_parent_module->sma_module_id }}"
                                                id="Main_Module_{{ $key }}"
                                                class="common-radio permission-checkAll main_module_id_{{ $student_parent_module->sma_module_id }}"
                                                {{ $check_sidebar != '' ? (in_array($student_parent_module->sma_module_id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                            <input type="hidden" value="{{ $student_parent_module->sma_module_id }}"
                                                name="all_modules_id[]">
                                            <label
                                                for="Main_Module_{{ $key }}">{{ __('menumanage::menuManage.' . $student_parent_module->name) }}</label>
                                        </div>
                                        @if (count($subModule) > 0)
                                            <div class="arrow collapsed" data-toggle="collapse"
                                                data-target="#Role{{ $student_parent_module->module_id }}">
                                            </div>
                                        @endif
                                    </div>
                                        ...
                                        <div class="permission_body">
                                            <ul class="submenuSort">
                                                @foreach ($subModule as $key => $row2)
                                                    <li>
                                                        <div class="submodule">
                                                            <input type="hidden" value="{{ $row2->sma_module_id }}"
                                                                name="all_modules_id[]">
                                                            <input id="Sub_Module_{{ $row2->sma_module_id }}"
                                                                name="module_id[]" value="{{ $row2->sma_module_id }}"
                                                                class="sma_csk common-radio  module_id_{{ $student_parent_module->sma_module_id }} module_link"
                                                                type="checkbox"
                            ...
                            <div class="single_role_blocks" data-id="menu_org{{ $student_parent_module->module_id }}">
                                <div class="single_permission" id="{{ $student_parent_module->id }}">
                                    <div class="permission_header d-flex align-items-center justify-content-between">
                                        <div>
                                            <input type="checkbox" name="module_id[]"
                                                value="{{ $student_parent_module->id }}"
                                                id="Main_Module_{{ $key }}"
                                                class="common-radio permission-checkAll main_module_id_{{ $student_parent_module->id }}"
                                                {{ $check_sidebar != '' ? (in_array($student_parent_module->id, $already_assigned) ? 'checked' : '') : 'checked' }}>
                                            <input type="hidden" value="{{ $student_parent_module->id }}"
                                                name="all_modules_id[]">
                                            <label
                                                for="Main_Module_{{ $key }}">{{ __('menumanage::menuManage.' . $student_parent_module->name) }}</label>
                                        </div>
                                        ...
                                        <div class="permission_body">
                                            <ul class="submenuSort">
                                                @foreach ($subModule as $key => $row2)
                                                    <li>
                                                        <div class="submodule">
                                                            <input type="hidden" value="{{ $row2->id }}"
                                                                name="all_modules_id[]">
                                                            <input id="Sub_Module_{{ $row2->id }}"
                                                                name="module_id[]" value="{{ $row2->id }}"
                                                                class="sma_csk common-radio  module_id_{{ $student_parent_module->id }} module_link"
                                                                type="checkbox"
                                ...
                                <div class="permission_header d-flex align-items-center justify-content-between">
                                    <div>
                                        ...
                                        <input type="hidden" value="{{ $student_parent_module->id }}"
                                            name="all_modules_id[]">
                                        <label
                                            for="Main_Module_{{ 10000 - $key }}">{{ __('menumanage::menuManage.' . $student_parent_module->name) }}</label>
                                    </div>
                                    @if (count($subModule) > 0)
                                        <div class="arrow collapsed" data-toggle="collapse"
                                            data-target="#Role{{ $student_parent_module->module_id }}">
