...
class Permission extends Model
{
    ...
    {
        return $this->belongsToMany(SmaRole::class, 'assign_permissions', 'permission_id', 'role_id');
    }
    ...
    public function childs()
    {
        ...
        return $this->hasMany(Permission::class, 'parent_route', 'route')
            ->when(!in_array($studentParent, [2, 3, 10]), function ($q) {
                $q->where('is_admin', 1);
            })->when($studentParent == 2 || 10, function ($q) {
    ...
    {
        return $this->hasMany('Modules\RolePermission\Entities\Permission', 'parent_route', 'route')->whereNotNull('route')->where('menu_status', 1);
    }
    ...
    public function scopeRoleWise($q)
    {
        ...
        return $q->whereNotNull('route')->where('route', '!=', '')
            ->when(in_array($role_id, [2, 10]), function ($q) {
                ...
                $q->where('is_parent', 1);
            })->when(!in_array($role_id, [2, 3, 10]), function ($q) {
                $q->where('is_admin', 1);
            });
    }
}
