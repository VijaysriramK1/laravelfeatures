    ...
    public function up()
    {
        ...
        try {
            ...
            foreach ($leadInfixModuleIds as $key => $data) {
                $check_exit = SmaModuleInfo::find($data[0]);
                if ($check_exit) {
                    continue;
                }
                $examPlan = new SmaModuleInfo;
                $examPlan->id = $data[0];
            ...
            foreach ($admins as $key => $value) {
                $admins_check = SmaPermissionAssign::where('module_id', $value)->where('role_id', 5)->first();
                $permission = new SmaPermissionAssign();
                $permission->module_id = (int)$value;
                $permission->module_info = SmaModuleInfo::find($value) ? SmaModuleInfo::find($value)->name : '';
