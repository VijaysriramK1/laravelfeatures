...
class FeesController extends Controller
{
    ...
    public function feesInvoice()
    {
        try {
            ...
            $smgroups = SmStudentGroup::where('school_id', Auth::user()->school_id)
                ->where('academic_id', getAcademicId())
                ->get();

            $studentCategorys = SmStudentCategory::where('school_id', Auth::user()->school_id)
                ->where('academic_id', getAcademicId())
                ->get();
            ...
            return view('fees::feesInvoice.feesInvoice', compact('studentCategorys', 'smgroups', 'classes', 'feesGroups', 'feesTypes', 'paymentMethods', 'bankAccounts', 'invoiceSettings'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function feesInvoiceStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->student != "all_student") {
                ...
                // $student = StudentRecord::where('student_id', $request->student)->get();
                ...
                // $storeTransaction->record_id = $request->student;
                ...
                //         $storeFeesInvoiceChield->fees_type = $group['feesType'] ?? 0;
                //         $storeFeesInvoiceChield->amount = $group['amount'] ?? 0;
                //         $storeFeesInvoiceChield->weaver = $group['weaver'] ?? 0;
                //         $storeFeesInvoiceChield->sub_total = $group['sub_total'] ?? 0;
                //         $storeFeesInvoiceChield->note = $group['note'] ?? '';


                //         if (isset($group['paid_amount'])) {
                //             $storeFeesInvoiceChield->paid_amount = $group['paid_amount']  > 0 ? $group['paid_amount'] : 0;
                //             $storeFeesInvoiceChield->due_amount = bcsub($group['sub_total'], $storeFeesInvoiceChield->paid_amount);
                //         } else {
                //             $storeFeesInvoiceChield->due_amount = $group['sub_total'] ?? 0;
                //         }
                ...
                //         $storeTransactionChield->fees_type = $transaction['fees_type'] ?? 0;
                //         $storeTransactionChield->weaver = $transaction['weaver'] ?? 0;
                //         $storeTransactionChield->paid_amount = $transaction['paid_amount'] ?? 0;
                //         $storeTransactionChield->note = $transaction['note'] ?? '';
                ...
                // $students = SmStudent::with('parents')->whereIn('id', $studentIds)->get();
                // $studentUserIds = $students->pluck('user_id');
                // $parentUserIds = $students->pluck('parents.user_id');
                // sendNotification("Fees Assign", null, $studentUserIds, 2);
                // sendNotification("Fees Assign", null, $parentUserIds, 3);
            } else {
    ...
    public function feesInvoiceEdit($id)
    {
        try {
            ...
            return view('fees::feesInvoice.feesInvoice', compact('studentCategorys','smgroups', 'classes', 'feesGroups', 'feesTypes', 'paymentMethods', 'bankAccounts', 'invoiceSettings', 'invoiceInfo', 'invoiceDetails', 'students'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
    ...
    public function feesInvoiceDatatable()
    {
        ...
        if (isset($studentInvoices)) {
            return FacadesDataTables::of($studentInvoices)
                ->addIndexColumn()
                ...
                ->addColumn('scholarship_amount', function ($row) {
                    return $row->TscholarshipAmount; 
                })
                ...
                ->addColumn('balance', function ($row) {
                    ...
                    $fine = $row->Tfine;
                    $paid_amount = $row->Tpaidamount;
                    $scholarship=$row->TscholarshipAmount;
                    $fees = $amount + $fine - ($paid_amount + $weaver);
                    $balance = $fees - $scholarship;
                    return $balance;
                })
                ->addColumn('status', function ($row) {
                    ...
                    } elseif ($paid_amount == 0 && $scholarship > 0) {
