...
<section class="admin-visitor-area up_st_admin_visitor student-details">
    <div class="container-fluid p-0">
        @if (isset($role) && $role =='admin')
            {{ Form::open(['class' => 'form-horizontal', 'method' => 'POST', 'route' => 'admissionfesspayment', 'enctype' => 'multipart/form-data']) }}
            <input type="hidden" name="record_id" value="{{$invoiceInfo->recordDetail->id}}">
        @else
            {{ Form::open(['class' => 'form-horizontal', 'method' => 'POST', 'route' => 'admissionfesspayment', 'id'=>'addFeesPayment','enctype' => 'multipart/form-data']) }}
                    ...
                    <div class="white-box radius-t-y-0">
                        <div class="single-meta mt-50">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="value">{{$invoiceInfo->studentInfo->name}}</div>
                            </div>
                        </div>
                        ...
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="value">{{$invoiceInfo->studentInfo->phone ?? "NULL"}}</div>
                            </div>
                        </div>
                        ...
                        <div class="single-meta">
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="value">{{$invoiceInfo->studentInfo->email ?? "NULL"}}</div>
                            </div>
                        </div>
                        ...
                        @if (moduleStatusCheck('MercadoPago') == true)
                            @include('mercadopago::form.userForm',['student_id' => $invoiceInfo->studentInfo->id, 'invoice_id' => $invoiceInfo->id,'fees'=>true])
                        @endif
                        <div class="row mt-40">
                                    ...
                                    @if (isset($invoiceInfo))
                                        ...
                                        @foreach ($invoiceDetails as $key=>$invoiceDetail)
                                            <tr>
                                                ...
                                                <td>{{$invoiceInfo->recordDetail->title}}</td>
                                                <td>
                                                    <div class="primary_input">
