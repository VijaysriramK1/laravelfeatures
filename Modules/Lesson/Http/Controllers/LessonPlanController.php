...
class LessonPlanController extends Controller
{
    ...
    public function topicOverview()
    {
        try {
            ...
            $query = SmLessonTopicDetail::with([
                'lesson_title.subject',
                'lesson_title.subject.assignedSubject.teacher',
                'lessonPlan',
                'lessonPlan.lessonDetail',
            ]);
            ...
            if ($user->role_id == 4) {
                ...
                $assign_subjects = SmAssignSubject::where('teacher_id', $smstaff->id)->groupby('class_id')->get();
                if (!$teacher_id) {
                    Toastr::error('Teacher profile not found', 'Failed');
                    return redirect()->back();
                }

                $query->whereHas('lesson_title.subject.assignedSubject', function ($q) use ($teacher_id) {
                    $q->where('teacher_id', $teacher_id);
                });
            }

            $topics_detail = $query->get();

            return view('lesson::lessonPlan.manage_lesson', compact('topics_detail', 'classes','assign_subjects','user'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
...
$user=Auth::user();
            ...
            return view('lesson::lessonPlan.manage_lesson', compact('lesson_id', 'classes', 'lessons', 'topics_detail', 'class_id', 'section_id', 'subject_id', 'selected','user'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
