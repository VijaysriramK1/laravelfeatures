...
class SmLessonController extends Controller
{
    ...
    public function loadLesson()
    {
        ...
        if (Auth::user()->role_id == 4) {
            ...
            ->get();
            $data['start_date'] = SmLesson::with('lessons', 'class', 'section', 'subject')
                ->statusCheck()
                ->select('class_id','section_id','subject_id','lesson_title','active_status','id','start_date')->groupBy(['class_id','section_id','subject_id'])
                ->get();
        } else {
            $data['lessons'] = SmLesson::with('lessons', 'class', 'section', 'subject')
                ->statusCheck()
