...
use Modules\VideoWatch\Entities\SmaVideoWatch;
...
class VideoWatchController extends Controller
{
    ...
    public function watchLog($id)
    {
        ...
        $watchLogs = SmaVideoWatch::where('study_material_id', $id)->get();
        foreach ($watchLogs as $key => $value) {
            $seen_students[] = $value->student_id;
        }

        $watchLogs = SmaVideoWatch::where('sma_video_watches.study_material_id', $id)
            ->leftjoin('sm_students', 'sm_students.user_id', '=', 'sma_video_watches.student_id')
            ->leftjoin('sm_teacher_upload_contents', 'sm_teacher_upload_contents.id', '=', 'sma_video_watches.study_material_id')
            ->select('sma_video_watches.*', 'sm_students.id', 'full_name', 'admission_no', 'roll_no', 'content_title')
            ->get();
    ...
    public function traceData(Request $request)
    {
        ...
        $check_exist = SmaVideoWatch::where('student_id', $request->user_id)->where('study_material_id', $request->study_id)->first();
        // if ($check_exist==null) {
        if (Auth::user()->role_id == 2 && $check_exist == null) {
            date_default_timezone_set(timeZone());

            $watch_trace = new SmaVideoWatch();
