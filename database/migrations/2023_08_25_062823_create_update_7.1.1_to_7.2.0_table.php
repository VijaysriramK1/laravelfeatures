...
return new class extends Migration
{
    public function up()
    {
        ...
        $s2 = SmaModuleManager::where('name', 'BehaviourRecords')->first();
        if (!$s2) {
            $s2 = new SmaModuleManager();
        }
        $s2->name = "BehaviourRecords";
        $s2->email = 'support@example.com';
        $s2->notes = "This is Behaviour Records Module for manage student behaviour records & Activity. Thanks for using .";
        $s2->version = "1.0";
        $s2->update_url = "https://example.com/contact";
        $s2->is_default = 0;
        $s2->purchase_code = time();
        $s2->addon_url = "https://sgupdate.test";
        ...
        $name = 'InAppLiveClass';
        $s = SmaModuleManager::where('name', $name)->first();
        if (!$s) {
            $s = new SmaModuleManager();
        }
        $s->name = $name;
        $s->email = 'support@example.com';
        $s->notes = 'This InAppLiveClass Module For SgEdu. Manage Online Class and Meeting Reports.';
        $s->version = "1.0";
        ...
        $notificationSettingMenus =  array(
