...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('sgaedu__pages', function (Blueprint $table) {
            if (!Schema::hasColumn('sgaedu__pages', 'is_default')) {
        ...
        $s = SmaModuleManager::where('name', 'InAppLiveClass')->whereNull('purchase_code')->count();
        if ($s > 1) {
            $del = SmaModuleManager::where('name', 'InAppLiveClass')->whereNull('purchase_code')->first();
            $del->delete();
        }
