...
class AddColumToSmaModuleStudentParentInfos extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('sma_module_student_parent_infos', function (Blueprint $table) {
            if (!Schema::hasColumn('sma_module_student_parent_infos', 'admin_section')) {
                $table->string('admin_section')->nullable();
            }
        });

        $examPlans = SmaModuleStudentParentInfo::whereIn('id', [2500, 2501, 2502, 2503])->delete();
        $sma_module_student_parent_infos = [
        ...
        foreach ($sma_module_student_parent_infos as $key => $value) {
            $check_exit = SmaModuleStudentParentInfo::find($value[0]);
            if ($check_exit) {
                continue;
            }
            $examPlan = new SmaModuleStudentParentInfo;
            $examPlan->id = $value[0];
        ...
        foreach ($ids as $key => $admin_section) {
            $permission = SmaModuleStudentParentInfo::where('id', $key)->first();
            if ($permission) {
